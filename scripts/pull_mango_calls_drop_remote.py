...
def pull_drop(*, remote_host: str, remote_drop_root: str, incoming_root: Path,
    ...
    transfer = Path(tempfile.mkdtemp(prefix=".transfer-", dir=local_incoming))
    ...
    try:
        ...
        if local_db is not None:
            return {"status": "reused", "sha256": expected_sha, "size_bytes": local_db.stat().st_size,
                    "mode": "manifest_only", "remote_files_received": 1,
                    "elapsed_sec": round(time.monotonic() - started, 3),
                    "transport": "read_only_ssh_manifest_only"}
        try:
            seed_db, _ = load_package(pipeline_root.expanduser().resolve() / "drop", exact=False)
            cloned = subprocess.run(["/bin/cp", "-c", str(seed_db), str(transfer / DB_NAME)],
