...
Постоянное дополнение M: поля реального времени в сравнении контекстов — известный класс флейков:

- если будущий STOP показывает diff в `*_state`, первым шагом грепать область diff'а на `updated_at`, `created_at`, `recorded_at`, `started_at`, `timestamp`, `*_at` и `datetime.now`;
- при подтверждении временного поля классифицировать как тестовую/измерительную нестабильность, если волна не трогала область сборки этого state;
- test-only фиксы должны замораживать время в конкретном модуле-владельце state через `monkeypatch.setattr`, без ослабления сравнения;
- replay canonical JSON должен маскировать реальные временные поля state, включая `updated_at` и временные поля внутри `slot_history`, чтобы baseline не зависел от пересечения границы секунды.
