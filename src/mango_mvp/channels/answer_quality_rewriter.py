...
def _deterministic_rewrite(
    ...
    *,
    ...
    if brand == "foton" and _asks_trial(client) and ("онлайн" in client or known.get("format") == "онлайн"):
        next_step = (
            "Данные уже вижу, повторно присылать их не нужно; передам менеджеру запрос на онлайн-пробное."
            if known.get("grade") and known.get("subject")
            else "Напишите класс и предмет — подберём подходящий вариант онлайн-пробного."
        )
        price_note = (
