...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Collect full phone history for several repaired messages(N) archives, exclude calls that "
            "already have transcription done in existing DBs, and build a new full-cycle batch."
        )
    )
    parser.add_argument("--archive-dir", action="append", required=True)
...
def main() -> int:
    args = _parse_args()
    archive_dirs = [Path(item).expanduser().resolve() for item in args.archive_dir]
    ...
    for archive_dir in archive_dirs:
        ...
        all_href_names.extend(Path(name).name for name in parser_html.hrefs)
        source_index_files += 1

    unique_href_names = list(dict.fromkeys(all_href_names))
    duplicate_ref_overflow = len(all_href_names) - len(unique_href_names)
    duplicate_ref_names = [name for name, count in Counter(all_href_names).items() if count > 1]

    archive_files: list[Path] = []
    archive_missing: list[str] = []
    archive_files_without_phone: list[Path] = []
    for name in unique_href_names:
        source_path = normalized_source_dir / name
        if not source_path.exists():
            archive_missing.append(name)
            continue
        phone = str(parse_filename_metadata(source_path.name).get("phone") or "").strip()
        if phone:
            archive_files.append(source_path)
        else:
            archive_files_without_phone.append(source_path)
    ...
    for path in all_history_files:
        ...
        if bool(state.get("transcription_done", False)):
            already_done += 1
            continue
    ...
    archive_calls_without_phone_csv = out_root / "archive_calls_without_phone.csv"
    manifest_path = out_root / "selection_manifest.json"
    prep_summary_path = out_root / "prep_summary.json"
    ...
    with archive_calls_without_phone_csv.open("w", encoding="utf-8", newline="") as fh:
        writer = csv.writer(fh)
        writer.writerow(["source_filename", "source_file", "manager_name", "started_at"])
        for path in archive_files_without_phone:
            meta = parse_filename_metadata(path.name)
            started_at = meta.get("started_at")
            writer.writerow(
                [
    ...
    archive_dates = _extract_dates(archive_files + archive_files_without_phone)
    ...
    manifest = {
        ...
        "archive_audio_refs": len(all_href_names),
        ...
        "archive_files_found_in_normalized_dir": len(archive_files) + len(archive_files_without_phone),
        "archive_calls_without_phone": len(archive_files_without_phone),
    ...
    prep_summary_path.write_text(
        json.dumps(
            {
                "archive_calls_without_phone": len(archive_files_without_phone),
                "csv": str(archive_calls_without_phone_csv),
            },
            ensure_ascii=False,
            indent=2,
        ),
