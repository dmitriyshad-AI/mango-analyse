...
### D-120. Mango Calls Phase A остаётся локальной, последовательной и fail-closed
...
### D-121. Часы, ACL, topology и dual-ASR проверяются на точке потребления

CLI внешнего watchdog всегда использует реальные часы хоста. Подмена времени
остаётся только аргументом внутренней Python-функции для unit-тестов и не
доступна оператору. Owner-only JSON читается через уже открытый descriptor;
mode `0600` не считается достаточным при наличии extended ACL. ACL проверяется
до и после чтения, а изменение ctime делает чтение недействительным. Код ничего
не исправляет автоматически и при неизвестном способе проверки отказывает.

Строгий M1 config нельзя запустить через старые команды wrapper без суффикса
`-worker`. Установщик требует явный `--fast-service`, `--process-a-only` или
`--process-b-only`, поэтому пропущенный topology-флаг не становится скрытым
fallback. Старый non-strict локальный контур сохраняет прежние команды только
для обратной совместимости и не является путём запуска M1. Wrapper передаёт
дочернему процессу SHA-256 уже проверенных байтов config; повторное чтение
другого поколения прекращает запуск до любой стадии конвейера.

Менеджерские XLSX и Google safe-plan самостоятельно проверяют две ASR-модели
либо полное явно утверждённое исключение. Одна расшифровка больше не считается
полностью обработанным звонком и всегда получает причину ручной проверки, даже
если Resolve и Analyze уже завершились. Stage 10 и Timeline сохраняют тот же
единый критерий. Координатор называет день `closed/final` только если сам
построенный пакет имеет `FINAL_CLOSED` и `closure_ok=true`; зелёный старый
verdict не может скрыть понижение фактических строк.

## 2026-08-14

### D-122. GigaAM используется выборочно и не подменяет основной Whisper-текст

Постоянный сервис распознаёт каждый звонок Whisper/MLX. GigaAM остаётся второй
независимой расшифровкой для содержательных, неоднозначных и рискованных
звонков, но не вызывается для уверенно определённых автоответчиков, IVR и иных
несодержательных контактов. Решение о пропуске принимает существующий
консервативный детектор `should_force_non_conversation`; ошибка, повреждённый
вход или неоднозначность всегда означают запуск GigaAM.

В сервисном режиме итоговая расшифровка по умолчанию строится из Whisper A.
GigaAM B сохраняется для rescue и проверки, но не смешивается с A автоматически;
если A пуст, B может восстановить текст. Решение записывается с версией и hash
политики. На один GigaAM worker арендуется один звонок, аренда продлевается на
каждом фрагменте, а финальная запись выполняется только при подтверждённом
владении. Потеря аренды прекращает только текущий звонок, а не постоянную службу.

Порог плотности Whisper-текста и детерминированная выборка пока используются
только как теневые метрики. Они не дают права пропускать GigaAM до отдельной
слепой проверки качества. Для controlled-прогонов прежний обязательный dual-ASR
контракт сохраняется.

### D-123. Рабочая Google-таблица звонков получает одного проверяемого publisher

Решение владельца 2026-08-15: natural-language writer заменяется одним
checked-in скриптом, а automation только запускает его. Capture, Whisper,
GigaAM, Resolve и Analyse не останавливаются и не переключаются. Старый и новый
writer никогда не пишут одновременно; потерянный ответ Google сначала
разрешается полным readback, а не повторным append.

Рабочая витрина сохраняет ровно 16 колонок A:P, включая телефон и
полную расшифровку. Это явное owner-решение для данного бизнес-листа, а не общее
разрешение публиковать ПДн в другие файлы. Q:Y после операции пусты, полная
расшифровка P визуально обрезана, высота строки определяется только конспектом
J. Локальный ledger хранится owner-only без телефонов и текстов; рабочая SQLite
не получает новую publication-схему.

Канонический J берётся из `summary`, затем `history_short`, и только затем из
`history_summary`. Это устраняет историческую смесь разных полей в одном
столбце. Если fallback-summary начинается с точного служебного UTC-времени
звонка, publisher переводит только этот префикс в МСК; время внутри содержания
разговора не переписывается. Фактический ACL листа на момент решения содержит
широкое право редактирования по ссылке; publisher его не меняет без отдельного
разрешения владельца, а риск должен быть закрыт отдельным ACL-cutover.

## 2026-08-17

### D-124. Доказательство ключевых выводов получает отдельный столбец

Решение владельца для Quality v3: новая версия витрины содержит 17 столбцов и
добавляет отдельное поле «Основание ключевых выводов» перед полем ручной
проверки РОПа. Это осознанно заменяет ограничение D-123 на 16 колонок: смешивать
доказательство и инструкцию РОПу в одной ячейке хуже для проверки качества.

Живой лист автоматически не мигрируется. Если на M1 обнаружены прежние A:P,
publisher остаётся STOP до отдельного плана вставки столбца, проверки формул,
ширин и полного readback, а затем отдельного разрешения Дмитрия. Остальные
границы D-123 — один writer, owner-only sidecar, отсутствие новой SQLite-схемы
и пустые неуправляемые колонки — сохраняются.

### D-125. Quality v3 экономит модельные вызовы без снижения качества на глаз

Analyse получает единственный канонический диалог в порядке реплик. Названия
сторон `Менеджер/Клиент` допустимы только при связанном с этим звонком
подтверждении Mango и совпадении текста с физическими дорожками. При отсутствии
такого доказательства сохраняются `Спикер A/B`, модель не вызывается, а
коммерческие выводы не публикуются. Это fail-closed защита, а не попытка угадать
сторону по приветствию, направлению или имени файла.

Экономия достигается только исключением ненужной работы: обычный Resolve идёт с
`RESOLVE_LLM_PROVIDER=off`, неподтверждённые и детерминированно несодержательные
звонки не вызывают Analyse-модель, неизменный вход переиспользует результат или
кэш, compact-профиль расширяется до full только по явному условию, Google
publisher не использует Codex. Каждая фактическая попытка учитывается по своему
provider/model/prompt_version; токены не оцениваются по символам.

Снижать модель, глубину или качество распознавания ради экономии запрещено до
слепого сравнения на замороженной выборке и отчёта с точными расходами. Первый
массовый исторический проход нового prompt-контракта остаётся `STOP` до пилота
50 звонков и заранее согласованного владельцем бюджета.

### D-126. Расход модели фиксируется до вызова, а публикация требует замкнутый журнал

Перед каждым внешним вызовом Analyse в `analysis_attempts_json` сначала
фиксируется уникальная попытка со статусом `reserved`. После ответа та же запись
становится `completed`, `failed` или `indeterminate`; новая запись для того же
вызова не создаётся. Потерянное подтверждение SQLite проверяется обратным
чтением. Ответ попадает в кэш только после подтверждённой записи итогового
анализа в БД.

Колонка `analysis_attempts_json` имеет приоритет над копией журнала внутри
`analysis_json`. Неопределённая, незавершённая или повреждённая попытка блокирует
внешнюю публикацию до первого и каждого повторного Google-запроса. Старые строки
без отдельного журнала читаются в режиме совместимости; новые записи обязаны
иметь уникальные `attempt_id` и закрытое состояние.

Технически невозможно обещать ровно один внешний вызов при аварии между
резервированием и отправкой: провайдеры не дают общего ключа идемпотентности.
Поэтому гарантия консервативная: неизвестная попытка не теряется и не выдаётся
за точный расход. Resolve не вызывает модель ни в штатном, ни в controlled
режиме: физические дорожки Mango уже задают стороны, а смысл извлекает Analyse.

### D-127. Выборочное смысловое слияние Resolve реализуется, но остаётся выключенным

Решение владельца: вариант A (один вызов модели только при доказанном расхождении
двух ASR) **принят к реализации**; постоянный вызов на каждом звонке **отклонён** до
эмпирического A/B. Владелец решения — Дмитрий.

D-127 разрешает **только код с `RESOLVE_SEMANTIC_MERGE_MODE = off` по умолчанию**.
D-127 **не разрешает**: обращения к Mango API и ASR, реальный Resolve/Analyze,
shadow-прогон, работу с данными с ПДн, любые runtime- и live-записи, включение
режима на M1. Каждое из этого — отдельное решение владельца. Само изменение env на
`selective` является включением и этим решением не выдано.

D-127 уточняет D-125: формулировка «Resolve не вызывает модель» становится
условной — «0 вызовов при `RESOLVE_SEMANTIC_MERGE_MODE=off`, что является дефолтом
службы и остаётся дефолтом до отдельного решения по итогам A/B». Остальное в D-125
и весь D-126 сохраняются без изменений.

Гарантию даёт не промпт, а детерминированный выходной сторож: модель предлагает
текст, а что попадёт в реплику — решает код. Роли, порядок, таймкоды, число реплик,
`variant_a` и `variant_b` не меняются никогда; числа, даты, валюты, отрицания,
квалификаторы и ФИО замораживают правку реплики; несловарная правка допускается
только как замена слова на близкое по написанию слово с той же первой буквой на
том же месте и без изменения пунктуации; словарная — только как посимвольно точная
подстановка `alias → canonical` из действующего tenant-нормализатора. Исходные
подписи `Менеджер/Клиент` восстанавливаются локально и модели не передаются. Любая
авария после гейта даёт baseline, а не падение стадии.

Сторож проверяет форму, а не смысл: близкий по написанию антоним и ФИО в нижнем
регистре без внешнего доказательства личности остаются риском. Поэтому pilot и live
остаются **BLOCKED**, а флаг `RESOLVE_SEMANTIC_MERGE_MODE` обязан быть удалён либо
зафиксирован как дефолт **не позже** решения по итогам слепого A/B на 100 звонках;
состояния «оставили висеть» нет.

### D-128. Первые 16 столбцов звонков неизменны, основания и UTM занимают 17–18

Решение владельца 2026-08-22 уточняет и в части порядка столбцов заменяет D-124:
рабочая витрина сохраняет прежние столбцы A:P без перестановок и изменения
смысла. `Основание ключевых выводов` занимает Q (17), `UTM и страница заявки` —
R (18), S:Y остаются пустыми. Остальные границы D-123 и D-124 сохраняются.

UTM читаются из amoCRM без записи. Прямая связь с заявкой считается доказанной
только после проверки, что телефон звонка принадлежит ровно одному связанному
контакту этой заявки. Связь по единственному точному контакту и ближайшей
предыдущей заявке явно помечается как предположение. При неоднозначности,
неполном ответе или недоступности amoCRM система показывает причину вместо
догадки. Сохранённые UTM — снимок карточки при первом подтверждённом
сопоставлении, а не обещание исторического состояния на момент звонка.

Живой лист автоматически не мигрируется. Переход разрешается только на новом
18-столбцовом листе по ступеням 1 → 10 → 25 строк с полным обратным чтением и
ручной проверкой РОПа. До отдельного разрешения владельца внешний запуск STOP.

### D-129. Mango не отдал официальную разметку ролей; Selective остаётся выключенным

Read-only проверка 22.08.2026 выполнена по лестнице 1 → 10 реальных
`recording_id`. Во всех ответах было только `{"result":5008}`; это доказывает
отказ метода для текущего ключа, но не различает тариф, права или настройку.
Записей во внешние системы, повторного ASR, Resolve и Analyse не было.

Код захвата сохраняет ролевые реплики Mango только после ответа `result=1000`,
точного совпадения запрошенного `recording_id`, сырого ответа и связи с конкретным
`source_call_id`. Mango не отдаёт готовый номер физической дорожки для роли:
дорожки выводятся строгим сопоставлением текстов и остаются недоверенными при
недостаточном совпадении. Ошибка API не теряет звонок: он продолжает путь без
доверенных ролей, а Selective Resolve не вызывает модель. Угадывать стороны по
номеру дорожки, приветствию, направлению или имени файла запрещено.

На замороженной выборке сохранённых пар Whisper/GigaAM лестница 20 → 50 → 100
дала 0 доверенных строк и 0 модельных вызовов. В исходной старой рабочей базе
также отсутствует независимый столбец `source_recording_id`, поэтому честное
сравнение старого и нового Resolve сейчас заблокировано источником данных, а не
качеством алгоритма.

Следующий запуск разрешён только после подключения услуги Mango и повторного
read-only ответа `result=1000`. Сначала проверяются новые звонки 1 → 10, затем
замороженный A/B 20 → 50 → 100 без повторного ASR. Автоматически добавлять новое
доказательство в уже полностью обработанные и опубликованные исторические строки
нельзя: их возврат в очередь является отдельной миграцией состояния с копией БД,
dry-run, балансом и отдельным разрешением Дмитрия. Рабочая служба M1, основной
Google-лист и дефолт `RESOLVE_SEMANTIC_MERGE_MODE=off` этим решением не меняются.

Сетевой захват разметки дополнительно закрыт временным флагом
`MANGO_PROVIDER_TRANSCRIPTS`, дефолт `off`, владелец — Дмитрий. До успешной
лестницы 1 → 10 флаг не включается. После замера 100 новых звонков Дмитрий
принимает одно из двух решений: сделать захват штатным и удалить флаг либо
удалить не подтвердившийся путь. Бессрочно оставлять неопределённый режим нельзя.

Промежуточное правило короткого ответа меняло десятки классификаций, в том числе
83 в одном из ранних замеров. Это число означало только отличие нового правила
от старого, а не 83 доказанные ошибки. Полный регрейд выявил более широкий
эффект, и вариант был отклонён. Финальная версия повторно проверена на всех
6 383 строках: короткие приветствия не повышаются до `service_call`, а остаются
в прежнем типе и при сомнении получают ручную проверку. Тип изменился только у
24 звонков: 16 роботов исправлены на `non_conversation`, восемь административных
звонков про рассылку документов — на `service_call`. Claude сначала проверил
18 строк, нашёл одну ошибку, после корневой правки финальные 17 подтвердились;
затем отдельно подтвердил 7/7 новых высокоточных роботов. Итог: 24/24 корректно.

### D-130. Связный конспект возвращён как отдельный display-only результат

Пилот 25.08.2026 показал, что строгий v3 правильно защищает критичные факты, но
в 17 из 25 содержательных звонков оставляет РОПу бесполезную заглушку. Один
существующий вызов Analyze снова формирует `manager_summary` на 3-5 предложений
по полному диалогу. Этот текст используется только в менеджерской витрине и не
подменяет проверенные поля оплаты, результата, возражений, срока и следующего
шага: они по-прежнему проходят отдельную проверку точных реплик.

UTM-резолвер, сырой кэш и счётчики связи с AMO не меняются. На границе Google
подробная техническая диагностика скрывается: сотрудник видит только реальные
`utm_*`, страницу заявки, `UTM не найдены` либо `UTM не определены`.
