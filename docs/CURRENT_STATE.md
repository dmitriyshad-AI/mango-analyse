...
1. Customer timeline получил read-only adapter, первый отрицательный coverage-аудит и локальное наполнение по 100 группам телефонов/сделок. Покрытие на этой выборке стало 100/100, но 82/100 групп требуют ручной проверки, поэтому флаги timeline пока не включаются.
...
После Блока E выполнен дополнительный read-only/local-write шаг:
