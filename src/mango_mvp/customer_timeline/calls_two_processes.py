...
def transcribe_environment(config: CallsTwoProcessesConfig, base: Mapping[str, str]) -> Mapping[str, str]:
    return {
        **base,
        ...
        "DUAL_MERGE_PROVIDER": "primary" if config.processing_scope == "service" else "rule",
        "GIGAAM_POLICY": "selective_non_conversation_v1" if config.processing_scope == "service" else "all",
