...
Для повторяющихся сборок скорость является частью `Done`, а не последующей
оптимизацией. По умолчанию сборка переиспользует уже сохранённый результат и
читает только новое или изменённое с безопасным перекрытием. Полный перечитывающий
проход допускается для первого запуска, восстановления или редкой проверки
полноты, но не как ежедневный штатный путь.

Каждый сетевой или массовый шаг обязан без ПДн сообщать: режим
`incremental/full`, число полученных, новых, изменённых, пропущенных и
переиспользованных записей, а также время чтения, обработки и записи. Второй
проход на неизменных данных должен быть почти пустым и не создавать дубли.
Перед ускорением сначала устраняй лишние скачивания и повторную обработку;
параллелизм и настройка базы применяются только после этого и только с
доказательством, что полнота и связи не ухудшились.
