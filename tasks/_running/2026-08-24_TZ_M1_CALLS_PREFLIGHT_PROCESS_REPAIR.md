...
4. Локальная сверка `git worktree list --porcelain` видит 37 деревьев:
   12 detached не участвуют в gate, текущая ветка исключается, остальные 24
   веточных worktree зарегистрированы; повторный preflight не сообщает ни об
   одном worktree вне реестра.
