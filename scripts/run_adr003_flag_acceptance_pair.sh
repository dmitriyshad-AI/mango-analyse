...
case "$TARGET_FLAG" in
  TELEGRAM_FIX1B_AUTONOMY_VERIFIED_FACTS|TELEGRAM_TEXT_HYGIENE_PAYMENT_FIX|TELEGRAM_DIALOG_SUMMARY_ROLLING)
    ;;
  *)
    echo "TARGET_FLAG must be one of TELEGRAM_FIX1B_AUTONOMY_VERIFIED_FACTS, TELEGRAM_TEXT_HYGIENE_PAYMENT_FIX, TELEGRAM_DIALOG_SUMMARY_ROLLING" >&2
    exit 2
    ;;
esac
...
base_env=(
  env
  -u TELEGRAM_FIX1B_AUTONOMY_VERIFIED_FACTS
