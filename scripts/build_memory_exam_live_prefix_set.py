...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    selected = select_dialogs(candidates, limit=args.limit, source_targets=parse_source_targets(args.source_target))
...
def build_dialog_candidates(
    ...
    *,
    ...
    for (source_system, customer_id, brand, dialog_key), messages in grouped.items():
        ...
        if not turns:
            continue
        visible_context_items = runtime_visible_context_items(
            timeline_db,
            customer_id=customer_id,
            brand=brand,
        )
...
def runtime_visible_context_items(db_path: Path, *, customer_id: str, brand: str) -> int:
    ...
    return len(items) if isinstance(items, Sequence) and not isinstance(items, (str, bytes, bytearray)) else 0
...
def render_micro_commands(
    ...
    *,
    ...
    common = (
        ...
        "--model gpt-5.5 --bot-reasoning high --judge-reasoning high "
        "--client-mode fake --memory-mode off --semantic-mode off --semantic-verifier-mode off"
    )
    return "\n".join(
        [
            "#!/usr/bin/env bash",
