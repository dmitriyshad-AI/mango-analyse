...
1. Связать deal-aware и каталог вопросов.
2. Затем встроить customer timeline как read-only источник истории.
3. Затем готовить следующий rebuild-раунд.
...
- Блок C реализован в текущем рабочем дереве и должен быть зафиксирован отдельным коммитом.
...
3. Рабочая папка перегружена параллельной разработкой, но границы зафиксированы.
4. Каталог скриптов устарел.
5. В `stable_runtime` много версий deal-aware артефактов без единой карты актуальности.
...
6. `D` - связь каталога вопросов и deal-aware gate. Следующий блок.
...
После коммита Блока C перейти к Блоку D:

- сохранить явные source-ключи каталога вопросов;
- построить связь между темами вопросов и deal-aware проверкой;
- блокировать конфликтные случаи между сервисными темами, manager-only правилами и продажным следующим шагом;
