...
Снимок для бота/симулятора: `.../kb_release_v3_snapshot.json` (схема v3; релиз v6.8; билдер v6.1).
...
`kb_release_20260813_v6_8_owner_approved_sources/` и применяет manifest владельца.

## ЧТО НЕЛЬЗЯ
НЕ запускать `scripts/build_kb_release_v3_from_claude_handoff.py`: у него старый `BUILDER_VERSION`,
дефолтный out-dir `v4`, и он НЕ применяет manifest текущего релиза → ложный `quality_passed=false` и риск порчи
...
3. `control_numbers_missing=[]`; число источников совпадает с manifest текущего релиза.
...
- релиз: v6.8 · схема снимка: v3 (`kb_release_v3_snapshot.json`) · билдер: v6.1.
