...
def build_semantic_output_verifier_prompt(
    *,
    ...
    return (
        ...
        "- Факт: «Фотон: курс проходит в группе». Ответ: «Группы маленькие, ребёнку будет комфортно». "
        "Вердикт: derived_product_claim, relation_to_base=adjacent — факт группы не подтверждает размер и комфорт.\n"
        "- Факт: «УНПК: в онлайн-курсе есть вебинары и домашние задания». "
        "Ответ: «Будет куратор, психологическая поддержка и места ещё есть». "
        "Вердикт: derived_product_claim, relation_to_base=absent — куратор, психподдержка и наличие мест не подтверждены.\n"
        "- Факт: «Фотон: перед стартом проводят распределение по уровню». "
        "Ответ: «Этот формат точно подойдёт тревожному ребёнку». "
        "Вердикт: individual_diagnosis, relation_to_base=absent — оценена пригодность для конкретного ребёнка.\n\n"
