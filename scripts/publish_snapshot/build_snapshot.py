...
from mango_mvp.customer_timeline.temporal import parse_aware_utc  # noqa: E402
...
def build_compact_reader(
    ...
    *,
    ...
    return {
        ...
        "size_headroom_bytes": max_size_bytes - snapshot_size,
        "within_size_limit": snapshot_size <= max_size_bytes,
    }
    ...
    *,
    ...
    try:
        with customer_timeline_run_lock(cfg.staging_db, timeout_seconds=lock_timeout) as run_lock:
            with customer_timeline_writer_lock(cfg.staging_db, timeout_seconds=lock_timeout) as writer_lock:
                ...
                if nightly_gate.get("ok") is not True:
                    ...
                    return report, False
                cutoff = parse_aware_utc(nightly_gate.get("published_at"))
                if cutoff is None:
                    report.update({"status": "failed", "nightly_manifest": nightly_gate, "reason": "invalid_cutoff"})
                    return report, False
                ...
                compaction = build_compact_reader(
                    cfg.staging_db,
                    temporary_db,
        ...
        source_freshness = {
            ...
            "required_sources_check": dict(nightly_gate.get("required_sources_check") or {}),
        ...
        manifest = {
            ...
            "publish_config_sha256": sha256_file(config_path),
            ...
            "schema_sha256": schema_signature_sha256(temporary_db),
        ...
        ok = bool(
            ...
            and reader_smoke_ok
            and source_freshness_evidence_ok(nightly_gate, source_freshness)
