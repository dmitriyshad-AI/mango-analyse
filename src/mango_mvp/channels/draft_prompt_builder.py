...
def _tone_sell_prompt_block(context: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        "- На первый содержательный вопрос дай максимум пользы сразу: прямой ответ из фактов + 1-2 близких полезных ориентира, "
        "но только если они совпадают по бренду, формату, классу, предмету и продукту с вопросом. Если scope не совпадает — не добавляй соседний факт.\n"
        "- На вопрос «за что платим», «чем вы отличаетесь от репетитора», «почему такая цена» отвечай как консультант: "
        "коротко разложи подтверждённые элементы ценности (формат, уровень группы, преподаватель, домашка/контроль, запись), "
        "не сравнивай агрессивно с репетиторами и не обещай результат.\n"
