...
**D1 status 2026-07-07 Step E:** function-level manifest создан: `docs/ADR003_DEAD_MONOLITH_FUNCTION_LEVEL_MANIFEST_20260707.md`. Физическое удаление мёртвого монолита в этом проходе отложено: импорт-аудит показал, что кандидаты мертвы для текущего direct-path, но всё ещё используются rollback/legacy/dialogue-contract/fake-provider/tests. Удалять до результата M1 и отдельного rollback-решения рискованнее, чем оставить как frozen legacy.
