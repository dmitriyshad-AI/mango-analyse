...
runner остаётся запрещён безусловно. Модельный текст отправляется только при
`route=bot_answer_self_for_pilot` и финальном `authoritative_output_gate=pass`;
иначе уходит фиксированный честный ответ без менеджерского текста.
...
Релиз содержит решения владельца от 13.08.2026 и расписание, проверенное
10.08.2026. Расписание действует по SLA только в день проверки; для ответа на
следующий день требуется штатная перепроверка источника и новый неизменяемый
релиз. Не подменять snapshot старыми v6.3/v6.7 из исторических документов. Бот может называть
