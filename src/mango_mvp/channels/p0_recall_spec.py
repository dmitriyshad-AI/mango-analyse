...
_CHILD_SAFETY_RISK_PATTERN = (
    r"(?:остал\w*\s+один|один\s+остал\w*|оставил\w*\s+одн\w*|"
    r"без\s+(?:присмотр\w*|надзор\w*)|потерял\w*|"
    r"никто\s+не\s+(?:следил|встретил\w*|подош[её]л\w*|забрал\w*))"
)
CHILD_SAFETY_COMPLAINT_RE = re.compile(
    rf"(?:{_CHILD_CONTEXT_PATTERN}[^.!?\n]{{0,120}}"
    rf"{_CHILD_SAFETY_RISK_PATTERN}"
    rf"|{_CHILD_SAFETY_RISK_PATTERN}"
    rf"[^.!?\n]{{0,120}}{_CHILD_CONTEXT_PATTERN}"
    rf"|после\s+заняти\w*[^.!?\n]{{0,80}}{_CHILD_SAFETY_RISK_PATTERN}"
...
P0_TRUE_POSITIVE_CASES: tuple[tuple[str, str], ...] = (
    ...
    *((message, "payment_dispute") for message in PAYMENT_DISPUTE_POSITIVE_CASES),
    ...
    ("Ребёнок один остался после занятия, никто не подошёл и не следил.", "complaint"),
    ("После занятия ребёнка никто не встретил.", "complaint"),
    ("После занятия оставили одного, никто не подошёл.", "complaint"),
...
P0_BENIGN_CASES: tuple[str, ...] = (
    ...
    *PAYMENT_DISPUTE_BENIGN_CASES,
    ...
    "Ребёнок один ходит на занятия по расписанию?",
