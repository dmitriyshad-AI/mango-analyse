...
_P0_PROMISE_GAP = r"(?:[ \t]+[^\s.!?]+){0,6}[ \t]+"


_P0_PROMISE_RE = re.compile(
    rf"(?:верн[её]м\b{_P0_PROMISE_GAP}деньг\w*\b|деньг\w*\b{_P0_PROMISE_GAP}верн[её]м\b)|"
    rf"(?:оформим\b{_P0_PROMISE_GAP}возврат\w*\b|возврат\w*\b{_P0_PROMISE_GAP}оформим\b)|"
    r"гаранти\w+\s+(?:результат|поступлен)|"
...
def _answer_mode_number_findings(
    ...
    *,
    ...
    fact_text = " ".join(str(value) for value in facts.values())
    backed_numbers = _numbers(fact_text) | _free_number_surfaces(fact_text)
    client_numbers = _numbers(client_message) | _free_number_surfaces(client_message)
