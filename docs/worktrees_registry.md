...
| `/Users/dmitrijfabarisov/Projects/Mango_timeline_canonical_identity` | `codex/timeline-canonical-identity` | Временный корневой фикс канонического клиента Tallanto/AMO, пока `main` занят длительным read-only/staging проходом Wappi. | После тестов и завершения текущего прохода влить в `main`, снять worktree и удалить ветку. |
