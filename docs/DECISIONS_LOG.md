...
### D-116. Часы, ACL, topology и dual-ASR проверяются на точке потребления
...
### D-117. GigaAM используется выборочно и не подменяет основной Whisper-текст
...
### D-118. Рабочая Google-таблица звонков получает одного проверяемого publisher
...
### D-119. Доказательство ключевых выводов получает отдельный столбец
...
проверки РОПа. Это осознанно заменяет ограничение D-118 на 16 колонок: смешивать
...
границы D-118 — один writer, owner-only sidecar, отсутствие новой SQLite-схемы
и пустые неуправляемые колонки — сохраняются.

### D-120. Quality v3 экономит модельные вызовы без снижения качества на глаз
...
### D-121. Расход модели фиксируется до вызова, а публикация требует замкнутый журнал
...
### D-122. Выборочное смысловое слияние Resolve реализуется, но остаётся выключенным
...
D-122 разрешает **только код с `RESOLVE_SEMANTIC_MERGE_MODE = off` по умолчанию**.
D-122 **не разрешает**: обращения к Mango API и ASR, реальный Resolve/Analyze,
...
D-122 уточняет D-120: формулировка «Resolve не вызывает модель» становится
условной — «0 вызовов при `RESOLVE_SEMANTIC_MERGE_MODE=off`, что является дефолтом
службы и остаётся дефолтом до отдельного решения по итогам A/B». Остальное в D-120
и весь D-121 сохраняются без изменений.
...
### D-123. Первые 16 столбцов звонков неизменны, основания и UTM занимают 17–18

Решение владельца 2026-08-22 уточняет и в части порядка столбцов заменяет D-119:
...
R (18), S:Y остаются пустыми. Остальные границы D-118 и D-119 сохраняются.
...
### D-124. Mango не отдал официальную разметку ролей; Selective остаётся выключенным
...
отказ метода для текущего ключа, но не различает тариф, права или настройку. Записей во внешние системы,
повторного ASR, Resolve и Analyse не было.
...
В реалистичном замере с полным сохранённым контекстом из 4 966 строк ручная
проверка выросла с 828 до 1 013: 190 строк добавлены, 5 сняты. Это безопасный,
но шумный слой: случайная проверка Claude показала, что часть очереди составляют
короткие обрывы и роботы. Явные шаблоны электронного секретаря автоматизированы,
а неоднозначные короткие контакты оставлены человеку. Старые данные и очередь
повторной обработки правило не меняет.

`MANGO_PROVIDER_TRANSCRIPTS` управляет только сетевым захватом новых ответов;
выключение флага не удаляет уже сохранённое доказательство. Полный откат
модельного улучшения обеспечивается `RESOLVE_SEMANTIC_MERGE_MODE=off`. До
повторного API-запроса M1 обязан проверить уже реализованный исполнимый
лимит/белый список 1 → 10, а до A/B — добавить точный журнал токенов Resolve.
Выборка A/B состоит из 100 новых
звонков после успешного захвата evidence, а парный Google-лист требует отдельного
писателя с ключом `(call_key, variant)`; основной publisher для этого не подходит.

Финальный независимый аудит Claude выявил четыре соседних риска, которые закрыты
до передачи M1: обычная рабочая аудиопапка больше не получает новые жёсткие
требования прав; межпартийный дубль `source_recording_id` лишает только доверия
к ролям и не роняет импорт; продажа с приехавшим клиентом не становится выдачей
ребёнка; текст модельной сводки не может сам объявить живой звонок роботом.
Расширенный контур из 1 275 тестов прошёл на Python 3.12. A/B сравнивает текущую
политику `off` целиком с новой экономной политикой `selective`: у них одинаковые
ASR-входы и provider evidence, но намеренно разные правила обращения к модели.
