...
то же ядро и отправляет содержательный модельный текст после общей выходной
проверки. Служебный маршрут модели не скрывает безопасное объяснение клиенту,
но сохраняет смысл для памяти и действий человека. Старый публичный runner не
используется.

Основные защитные правила:

- P0: реальные возвраты, споры оплаты, серьёзные жалобы и юридические вопросы
  помечаются как требующие человека; публичный бот даёт только честный безопасный
  ответ и официальный способ связи, а Wappi создаёт черновик менеджеру;
...
- маршрут: модель определяет необходимость человеческого действия, а общий
  выходной гейт отдельно решает, можно ли показать подготовленный текст клиенту;
- действие: публичный Telegram не обещает передачу менеджеру, пока такого
  технического канала нет.
