...
| `/Users/dmitrijfabarisov/Projects/Mango_global_code_cut_wave10_20260805` | `codex/global-code-cut-wave11`; SHA через `git rev-parse HEAD` | Уборка одноразовых архивных/history-скриптов, заменённых каноническим calls/mail/Timeline-конвейером. | После audit pack, слияния в `main` и отдельного подтверждения владельца. |
...
| `/Users/dmitrijfabarisov/Projects/Mango_timeline_owner_relink_fix_20260805` | `codex/timeline-tallanto-id-classification`; SHA через `git rev-parse HEAD` | Исправление ложной классификации технического master-contact ID как Tallanto student ID. | После тестов, audit pack, слияния в `main` и отдельного подтверждения владельца. |
