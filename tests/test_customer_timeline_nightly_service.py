...
def test_canonical_nightly_requires_verified_writer_ownership_receipt(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(ValueError, match="config SHA mismatch"):
        validate_writer_ownership(replace(canonical, source_config_sha256="d" * 64))

    manually_active = json.loads(receipt_path.read_text(encoding="utf-8"))
    manually_active["status"] = "M4_WRITER_ACTIVE"
    ...
    with pytest.raises(ValueError, match="invalid transfer state"):
        validate_writer_ownership(canonical)
    manually_active["status"] = "TRANSFERRED"
    receipt_path.write_text(json.dumps(manually_active), encoding="utf-8")
    ownership = validate_writer_ownership(canonical)
...
def test_canonical_timeline_path_cannot_bypass_ownership_with_short_source_list(tmp_path: Path) -> None:
    config = service_config_from_json(write_service_config(tmp_path))
    canonical_path = tmp_path / "customer_timeline_staging.sqlite"
    canonical_path.write_bytes(b"seed")

    with pytest.raises(ValueError, match="WRITER_OWNERSHIP"):
        validate_writer_ownership(
            replace(
                config,
                timeline_db=canonical_path,
...
def test_nightly_service_fails_closed_when_family_conflict_reconciliation_breaks(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    staging = tmp_path / ".codex_local" / "staging"
    staging.mkdir(parents=True)
    ...
    def break_reconciliation(*_args, **_kwargs):
        raise RuntimeError("family reconciliation negative control")

    monkeypatch.setattr(family_graph_module, "_reconcile_contact_conflicts", break_reconciliation)
    config_path = staging / "service.json"
    config_path.write_text(json.dumps({
        "timeline_db": str(db_path),
        "allowed_root": str(staging),
        "out_root": str(staging / "runs"),
        "publish_dir": str(staging / "published"),
        "steps": [{"name": "family_graph_refresh", "kind": "family_graph", "required": True, "config": {"apply": True}}],
    }), encoding="utf-8")
