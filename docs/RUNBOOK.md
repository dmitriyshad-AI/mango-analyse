...
Решением D-105 публичный бот исключён из целевой архитектуры и удалён из
актуального `main`. Запуск из старого worktree запрещён безусловно. Если
`scripts/skills/live_truth.py --no-write` находит старый публичный runner, его
статус обязан быть `WARN`; любые live-действия останавливаются до отдельного
решения владельца.
