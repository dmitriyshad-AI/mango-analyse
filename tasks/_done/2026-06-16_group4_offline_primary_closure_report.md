...
Дата: 2026-06-16  
Ветка текущего отчета: `codex/tz121-group4-remaining`
...
| D — роли моно-звонков | `primary` разрешён | `codex_selective` только на low-confidence; уверенное правило остается правилом; segment guard не применять |
...
- D: commit `ce836b0` на ветке `codex/tz118-group4-primary-d`, отчет `tasks/_done/2026-06-16_TZ118_block_D_primary_enable_report.md`
...
- D: принятый регрейд — `94.62%` средняя точность по репликам против около `55%` у слабого правила; остаточные модельные сегментные ошибки признаны приемлемым офлайн-риском.
...
D-ветка `codex/tz118-group4-primary-d` по отчету commit `ce836b0`:

```text
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src python3 -m pytest -q

3300 passed, 5 skipped, 1 warning
```
