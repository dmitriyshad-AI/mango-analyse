...
1. Расширять AMO payload коммерческими полями.
2. Затем структурировать возражения.
...
- Блок PBF реализован в текущем рабочем дереве и должен быть зафиксирован отдельным коммитом.
...
1. Коммерческие признаки пока не дошли до deal-aware AMO payload как отдельные поля.
2. Customer timeline полезен, но еще не встроен как основной read-only источник истории.
3. Каталог вопросов и deal-aware quality gate слабо связаны.
...
4. `B` - коммерческие поля. Следующий блок.
...
После коммита Блока PBF перейти к Блоку B:

- добавить коммерческие поля в deal-aware payload;
- держать новые AMO-поля optional до создания полей в AMO;
