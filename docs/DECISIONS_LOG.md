...
ребёнка в `family_graph_v1`. Слияние разрешено только внутри одного
`customer_id`: полные имена должны совпасть минимум по двум токенам с учётом
локальных детских алиасов (`Дан/Даня/Даниил/Данил/Дениил`, `Лиза/Елизавета`)
и ограниченных опечаток/склонений фамилии. Однословное прозвище присоединяется
только если есть ровно один многословный кандидат. Слабые одноразовые кандидаты
в шумной `identity_risk`-семье не удаляются из БД, а помечаются
`excluded/suspicious_child_name`, если выглядят как дубль более подтверждённого
ребёнка.
...
Проверка: staging-пересчёт дал `family_links_total 8408 -> 7057`,
семьи с `>=4` детьми `189 -> 45`, подозрительные `>=4` singleton-evidence
семьи `175 -> 10`, `high_in_ge4=0`, `quick_check=ok`, JSON валиден. Gold v1:
