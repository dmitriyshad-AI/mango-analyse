...
На таком эталоне модель улучшает один случай, но один уверенный модельный вывод остается неверным.

Итоговое решение после регрейда/приёмки: A primary не включать. Разница `23/24` у модели против `22/24` у правила слишком мала и выглядит как шум, а `1` уверенная ошибка модели делает primary небезопасным. Блок A остается в `shadow`.
...
Если потребуется пересмотреть A, безопасный следующий шаг — добрать более крупный gold-набор по сделкам и заново сравнить модель против правила. До такого отдельного решения A остается `shadow`.
