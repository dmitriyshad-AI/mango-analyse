...
## Разрешенное обновление 2026-07-30: граница регистра в проверках имени

Исправлены восемь существующих проверок имени в `dialogue_memory.py`, `direct_path.py`, `post_layers.py` и `next_step_resolver.py`. Глобальный `re.I` раньше распространялся на группу `[А-ЯЁ]`, поэтому обычные строчные слова после маркеров (`для занятий`, `я хочу`, `ребенка полезнее`) могли ошибочно считаться именами клиента или ребёнка, а отрицание `Менеджер не отправит` превращалось в активный шаг `Отправит`. Две проверки direct-path дополнительно склеивали имя с заглавным словом на следующей строке; внутри имени теперь разрешены только пробел и табуляция.

Новых регулярных выражений не добавлено. Нечувствительность к регистру сужена до слов-маркеров, а имя снова обязано начинаться с заглавной буквы. Существующий стоп-лист имени дополнен только семейными ролями, чтобы `Ребёнок` или `Дочка` в начале предложения не записывались именем. Регрессионные тесты проверяют отрицательные и положительные примеры. Изменение относится к точности существующей памяти и ПДн/output-защиты, не выбирает тему, продукт, маршрут или автономность.

Snapshots `tests/fixtures/adr003_runtime_channel_regex_snapshot.json` и `tests/fixtures/adr003_direct_path_text_patterns_snapshot.json` обновлены намеренно.

## Разрешенное обновление 2026-07-31: запасной пол денежного обещания

Один существующий выходной шаблон `_P0_PROMISE_RE` теперь допускает промежуточные слова и обратный порядок между уже известными частями `вернём` и `деньги`. Новых слов понимания и новых регулярных выражений нет: смысловые перифразы денежного обещания определяет уже существующий верификатор финального черновика кодом `p0_money_promise`, а шаблон остаётся узким запасным рубежом при выключенной или недоступной модели. Итоговый гейт блокирует этот код через уже существующий канал `metadata.semantic_output_verifier`; отдельный передатчик не добавлялся.

Изменение относится только к выходному P0-полу. Оно не классифицирует входящий запрос, не повышает автономность и не меняет брендовые, числовые или ПДн-правила. Snapshot `tests/fixtures/adr003_runtime_channel_regex_snapshot.json` обновлён намеренно; число записей осталось прежним.
...
## Разрешенное обновление 2026-07-28: удаление двух точных PII-regex дублей

Локальные имена `_SEMANTIC_FRAME_PHONE_RE` и `_SEMANTIC_FRAME_EMAIL_RE` в
`provider.py` сохранены, но теперь ссылаются на уже импортированные канонические
объекты `_A2_PHONE_RE` и `_CLIENT_EMAIL_RE` из `support.py`. Тексты шаблонов и
флаги совпадали побайтово; клиентское понимание, маскирование, маршруты и
публичные имена не изменены. Из frozen snapshots удалены только два повторных
объявления, а regex-бюджет `provider.py` снижен с 3 до 1.

## Разрешенное обновление 2026-07-30: единый фильтр ПДн для памяти клиента

Удалён локальный `_BOT_SAFE_PERSON_CONTEXT_RE` из `direct_path.py`: проверка
теперь вызывает тот же `scan_bot_safe_context_pii`, которым фильтруются куски
Customer Timeline. Это выходной стоп-фильтр ПДн, а не распознавание смысла
сообщения. Единый владелец также не считает глаголы после слова «менеджер»
именами людей; это закреплено сквозными тестами памяти и direct-path.
Frozen snapshots обновлены только на удаление одного повторного объявления;
новых regex, смысловых правил и вызовов модели не добавлено.

## Разрешенное обновление 2026-08-04: fail-closed контракт `is_p0`

`provider.py` теперь различает физический JSON boolean, отсутствующее поле и
невалидный тип обязательного модельного поля `is_p0`. Это проверка схемы уже
полученного ответа модели: она не читает и не классифицирует клиентский текст,
не добавляет regex, marker-таблицу, P0-класс, флаг или вызов модели. При
missing/invalid результат закрывается к менеджеру без ложной P0-защёлки.
Соседние `risk_level` и `p0_kind` не могут повысить физический `false` до P0.

Snapshot `tests/fixtures/adr003_direct_path_text_patterns_snapshot.json` и номера
строк в `docs/adr003_understanding_map.yaml` обновлены механически из-за сдвига
существующих узлов ниже новой fail-closed проверки. Состав снимка остался
прежним: 832 записи; смысловые классы и regex/marker-бюджеты не менялись.

Откат в профиле `pilot_gold_v1` выполняется только парой существующих флагов:
`TELEGRAM_P0_MODEL_LED=0` и `TELEGRAM_DIRECT_PATH_MODEL_P0=0`. Один выключенный
флаг не отключает второй путь и не является доказательством отката.

## Разрешённое обновление 2026-08-05: удаление повторного смыслового прохода

Из живого `build_draft` удалены повторное применение старого
`conversation_intent_plan`, второй route-rubric вызов модели и шаблонные
переписчики near-repeat/next-step после уже полученного модельного ответа.
Детерминированные слои сохраняют P0, принадлежность клиента и проверку фактов,
но больше не сочиняют другой ответ. Дополнительно удалены два дублирующих
regex-переписчика служебных фраз: их риски уже проверяет единый финальный гейт.
Снимок уменьшился с 832 до 809 строк; новых regex и marker-правил нет.
Frozen-бюджет `post_layers.py` снижен с 72 до 70.

## Разрешённое обновление 2026-08-06: модель владеет смыслом P0

После экзамена M1 на 815 случаях и решения D-113 входная P0-классификация в
живом direct path больше не использует regex, тему или словарный preblock.
Обязательные модельные поля `is_p0` и `p0_kind` приходят в существующем вызове;
missing/invalid остаётся fail-closed. Сохранены ранее подтверждённая защёлка
диалога и выходные проверки обещаний, фактов и принадлежности клиента.

Удалён complaint-backstop и две поглощённые P0-записи frozen-карты. Runtime
regex-снимок уменьшился на одну запись, direct-path снимок — с 809 до 807.
Новых regex, marker-таблиц, флагов, зависимостей и модельных вызовов нет.
Существующее имя `TELEGRAM_P0_MODEL_LED` дополнительно объявлено в памяти
диалога, чтобы один и тот же режим применялся до и после модельного ответа.

## Решение 2026-08-04: смысловой retriever вместо новой грамматики цены

В `pilot_gold_v1` включены уже существующие `TELEGRAM_RETRIEVER_MODEL_DRIVEN`
и `TELEGRAM_ASSUMED_SCOPE_GUARD`. Дополнительного вызова модели, флага,
регулярки или словаря нет: тот же retriever-вызов сам определяет нужные факты.
Чтобы старая регулярка не подсказывала модели готовый ответ, в model-driven
prompt больше не передаются `primary_intent`, `answer_topics` и
`required_fact_keys`.

Пустой выбор, невалидный ответ или недоступность retriever в model-driven
режиме закрываются без фактов. Keyword-отбор не используется как аварийное
смысловое понимание: он доступен только при явном откате модельного режима. Это
устраняет доказанный класс, где отсутствие подходящей цены превращалось в
десятки нерелевантных фактов о скидках, расписании и лагере.

Откат несимметричен и намеренно явный: `TELEGRAM_RETRIEVER_MODEL_DRIVEN=0`
возвращает прежний retriever-контракт с подсказками планировщика;
`TELEGRAM_ASSUMED_SCOPE_GUARD=0` также выключает model-driven слой. Полный
возврат к keyword-отбору требует `TELEGRAM_LLM_RETRIEVE=0`. Любой такой
OFF-прогон уже не является каноническим `pilot_gold_v1` и запускается только
как отрицательный контроль с явным разрешением неполного профиля.

В том же SemanticFrame-вызове добавлен строгий boolean
`open_question_unanswered`. Он используется только как запрет косметическому
tone-close стирать содержательный ответ; менеджерские маршруты остаются
монотонными. Произвольная модельная `metadata` целиком не считается доверенным
источником: служебные метки и гейты собираются только локальным кодом из
нормализованных физических полей ответа.

Regex-бюджеты и состав frozen inventory не изменились: 832 строки и те же
832 устойчивых `row_id`. Snapshot и карта обновлены механически из-за сдвига
номеров строк. Служебный literal-membership счётчик также не изменился.

## Разрешённое обновление 2026-08-11: удаление повторных владельцев смысла

Из живого direct path и связанных измерителей удалены невызванные матрица
автономности, повторный `conversation_intent_plan` guard, `deal_action`,
live-status/intent-actions переходы, keyword-reask, tone-close и sell-observer.
Они повторно классифицировали текст после уже полученного SemanticFrame либо
сохраняли телеметрию без бизнес-потребителя. Отдельный
`manager_handoff_summary.py` также удалён: конечная AMO-заметка теперь показывает
существующие `requested_action`, `manager_checklist` и `missing_facts` основной
модели, не создавая второй контракт действий.

Рабочий код: `+56/-2926` строк. Runtime regex snapshot уменьшен `187 -> 163`,
общий direct-path inventory `807 -> 735`, бюджет marker-helper в
`policy_routing.py` `17 -> 6`. Новых runtime-файлов, флагов, зависимостей,
регулярок и модельных вызовов нет. P0, output/fact, identity, ПДн и запрет
неподтверждённых live-обещаний не удалялись.

Pre-model `conversation_intent_plan` пока остаётся: он участвует в выборе точных
фактов до основной модели. Имеющийся Wappi replay содержит 25 непривязанных к
