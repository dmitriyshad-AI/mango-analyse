...
| `/Users/dmitrijfabarisov/Projects/Mango_question_catalog_cleanup_20260806` | `codex/conversation-topic-taxonomy-contract`; SHA через `git rev-parse HEAD` | Исправление канонического topic_id темы преподавателя без нового слоя. | После тестов, audit pack, слияния в `main` и отдельного подтверждения владельца. |
