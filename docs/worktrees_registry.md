...
| `/Users/dmitrijfabarisov/Projects/Mango_business_journey_traces_wave1_20260803` | `codex/business-journey-traces-wave1-20260803`, чистый | Донор проверен: полезный сквозной сценарий поглощён выборочно; прямое вливание заблокировано из-за дубля трассировки и ложного коммерческого факта. | Не переключать и не изменять; после подтверждения владельца можно отдельно снять ветку и worktree. |
| `/Users/dmitrijfabarisov/Projects/Mango_business_journey_integration_20260803` | `codex/business-journey-integration-20260803`, завершённый | Два герметичных сквозных бизнес-теста без production-правок; архитектор, ломатель и бизнес-аудитор дали PASS. | После fast-forward в `main` и оба зеркала запросить снятие; ветку/worktree удалить только после подтверждения владельца. |
| `/Users/dmitrijfabarisov/Projects/Mango_noncontentful_call_memory_integration_20260804` | `main`, активная последовательная интеграция | Доноры Timeline, business journey, call-dialogue-timing и P0 model-led обработаны последовательно; полезное поглощено выборочно, прямые merge отклонены как дублирующие или менее строгие. | Использовать для следующего донора после фиксации результата; удалять worktree только по решению владельца. |
| `/Users/dmitrijfabarisov/Projects/Mango_critical_gates_20260731` | `codex/call-dialogue-timing-integration-20260803`, завершённый | Полезные таймкоды, происхождение реплик и строгий `manager_ready` уже в `main`; остаток старой ветки не нужен. | После подтверждения владельца можно отдельно снять ветку и worktree. |
| `/Users/dmitrijfabarisov/Projects/Mango_customer_timeline_junk_map_20260731` | `codex/customer-timeline-junk-map-20260731`, проверенный донор | Прямое вливание отклонено: Telegram-гейт слабее `main`, а инвентаризатор не меняет поведение и ошибочно использует `immutable=1` при WAL. Полезная бизнес-цель поглощена минимальным корневым решением. | После публикации интеграции и проверки отсутствия процессов можно запросить у владельца снятие worktree и удаление ветки. |
...
| `/Users/dmitrijfabarisov/Projects/Mango_p0_model_led` | `codex/p0-model-led`, проверенный донор | Прямое слияние не требуется: код донора поглощён `main@fce2bdc1`, а более строгий fail-closed контракт и честный M1-измеритель приняты отдельной последовательной интеграцией. | Не использовать для M1 и не менять; живой экзамен запускать на новом `main`. После его решения и подтверждения владельца можно снять worktree и удалить ветку. |
| `/Users/dmitrijfabarisov/Projects/Mango_tallanto_linkage` | `codex/tallanto-linkage-root`, поглощён | Патч донора идентичен `main@a1a26ac5`; его незакоммиченные продолжения заменены более строгими `564d6c95` и `8d5f38ad`. Три уникальных audit pack и точный dirty patch сохранены в последовательном integration-worktree. | Не использовать для разработки. Снять после отдельного решения о двух локальных Telegram-журналах; код и тесты переносить не нужно. |
...
| `/Users/dmitrijfabarisov/Projects/Mango_call_dialogue_timing_20260728` | `codex/call-dialogue-timing-20260728`, проверенный донор | Полезная часть уже поглощена в `main`; прямое слияние отклонено из-за приблизительной хронологии, отдельного ASR-recovery и ПДн в именах файлов. | После подтверждения владельца можно отдельно снять ветку и worktree. |
...
| `/Users/dmitrijfabarisov/Projects/Mango_wappi_ct_minimal_fix_20260729` | detached `d9dd3cb4`, грязный чужими правками | Изолированная доработка Wappi/Customer Timeline другого исполнителя; текущая интеграция звонков её не использует. | Не переключать, не чистить и не изменять; владелец правок должен отдельно завершить или передать пакет. |
