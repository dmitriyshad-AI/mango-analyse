...
| `/Users/dmitrijfabarisov/Projects/Mango_timeline_owner_relink_fix_20260805` | `codex/timeline-tia-reconcile-20260806`; SHA через `git rev-parse HEAD` | Переоценка старых Tallanto ambiguity для уже подтверждённого exact-владельца; только существующий importer, тесты и APFS-клон staging. | После лестницы 1→10→всё, audit pack и слияния в `main`. |
