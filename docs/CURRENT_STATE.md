...
1. Структурировать возражения.
2. Затем связать deal-aware и каталог вопросов.
3. Затем встроить customer timeline как read-only источник истории.
...
- Блок B реализован в текущем рабочем дереве и должен быть зафиксирован отдельным коммитом.
...
1. Customer timeline полезен, но еще не встроен как основной read-only источник истории.
2. Каталог вопросов и deal-aware quality gate слабо связаны.
3. Структурные возражения еще не выделены отдельным машинным слоем.
4. Рабочая папка перегружена параллельной разработкой, но границы зафиксированы.
5. Каталог скриптов устарел.
6. В `stable_runtime` много версий deal-aware артефактов без единой карты актуальности.
...
5. `C` - структурные возражения. Следующий блок.
...
После коммита Блока B перейти к Блоку C:

- сохранить человекочитаемое поле возражений;
- добавить внутренний структурный список возражений;
- не писать JSON в AMO до появления реального потребителя;
- не запускать live AMO/Tallanto/CRM и не менять `stable_runtime`.
