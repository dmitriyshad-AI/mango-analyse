...
_CHILD_NAME_MARKER_RE = re.compile(
    r"\b(?i:(?:сына|сын|дочь|дочку|дочк[аеуы]|реб[её]н(?:ка|ок))\s+зовут)\s+(?P<name>[А-ЯЁ][а-яё]{2,20})\b"
    r"|\b(?i:(?:зовут|фио|для))\s*[:\-]?\s*(?P<name2>[А-ЯЁ][а-яё]{2,20})\b",
)
...
_NAME_GRADE_RE = re.compile(r"\b(?P<name>[А-ЯЁ][а-яё]{2,20})\s+(?i:в)\s+(?:[1-9]|1[01])\s*(?i:(?:класс|кл\.?))\b")
