...
- literal вызовы `has_marker` / `has_any_marker` / `has_word_marker` и их локальные `_...`-алиасы;
...
## Разрешённое обновление 2026-08-13: канонический model-owned direct path

В профиле `pilot_gold_v1` старые keyword/regex-планы, предвыбранные темы и
готовые наборы фактов удаляются из контекста до model-driven retriever и
основного direct-path вызова. Смысл реплики и `requested_action` теперь принадлежат
существующей модели, а выбранное действие доходит до заметки менеджеру.

Удалён keyword-подбор gold-примеров: остаются два стабильных примера стиля без
классификации текущего текста. Новых regex, флагов и вызовов модели нет.
Проверки P0-защёлки, identity/ПДн, фактов и опасных обещаний сохранены как
детерминированный выходной пол. Явное отключение одного из трёх канонических
условий оставляет прежний контекст без изменений.

Frozen snapshot обновлён только из-за сдвига координат существующих узлов после
удалений и добавления model-boundary; состав остаётся 697 строк, row ID и полная
классификация не изменились. Три обезличенных read-only прогона реального
provider подтвердили смысл, выбор факта и менеджерское действие без внешних
записей.
...
Обновление 2026-08-12: это ограничение снято для канонического
`pilot_gold_v1`. Рабочий provider перед модельным retriever удаляет из своей
локальной копии контекста старые `conversation_intent_plan`, `answer_contract`,
заранее выбранные факты и смысловые поля памяти. Полный read-only snapshot,
точная принадлежность клиента, подтверждённые слоты и P0-защёлка сохраняются.
Retriever сам выбирает необходимые факты и при ошибке закрывается без keyword-
fallback. Явный `TELEGRAM_LLM_RETRIEVE=0` оставляет старый режим только как
rollback-контроль. Исходный context не мутируется, поэтому накопление памяти и
CRM-заметка продолжают использовать структурные данные текущего диалога.

Inline `SemanticFrame` теперь помечается `active`: его `requested_action`
становится следующим шагом в AMO-заметке, а модельный `route` должен быть с ним
согласован. Детерминированные P0/output/fact/identity/ПДн-полы могут только
ужесточить маршрут или очистить опасный текст; второй пониматель смысла не
добавлен. Новых regex, marker-таблиц, флагов, зависимостей и LLM-вызовов нет.
...
Direct-path inventory уменьшен `735 -> 697`: удалено 26 inline regex-вызовов,
...
В каноническом model-led профиле подбор примеров стиля больше не классифицирует
вопрос по ключевым словам: модель получает два стабильных брендовых примера
только как образец тона. Старые `planner_intent` и `dialogue_contract_pipeline` также не
попадают в модельный путь. Явный rollback с выключенным retriever оставляет старую
схему без изменений. Так удалена ещё одна смысловая string-membership проверка.
...
## Разрешённое обновление 2026-08-13: актуализация рабочей базы знаний

При переводе runtime на подтверждённый владельцем релиз удалены четыре отдельные
проверки слов-маркеров летних школ и одна локальная проверка `valid_until`.
Маркеры сведены в существующий технический lookup, а единый временной гейт теперь
проверяет `valid_from`, `valid_until` и дату свежести. Новых смысловых правил,
regex, флагов или модельных вызовов нет. Direct-path inventory уменьшен
`697 -> 692`; literal-membership inventory уменьшен `289 -> 285`.

После финального удаления test-only обёртки и старого летнего fallback
literal-membership inventory уточнён по актуальному AST: `289 -> 282`
(`142` в канонической текстовой зоне, `140` вне неё). Состав frozen inventory
остался `692` строки; изменились только координаты после удаления кода.

## Разрешённое обновление 2026-08-13: очистка публичного ответа и снятие повторного зажима

Существующие технические scrub-паттерны расширены только на внутренние поля
`internal_trace_id`, `model`, строку `DEBUG: route=...` и JSON со
`safety_flags`. Это не понимание клиентского запроса, а последняя механическая
очистка уже готового текста перед публичным Telegram-ответом.

После успешной регенерации и повторной проверки смысловой verifier больше не
понижает разрешённый модельный маршрут до менеджерского: второй смысловой
вердикт не создаётся, а все P0/output/fact/identity/ПДн-полы остаются на месте.
Число runtime regex и строк direct-path inventory не выросло: `163` и `692`.
Frozen snapshots обновлены намеренно на два изменённых технических паттерна и
механический сдвиг координат.

## Разрешённое обновление 2026-08-14: удаление матрицы автономности

Удалены две одинаковые таблицы `AUTONOMY_MATRIX_SAFE_TOPIC_IDS` и их мёртвая
обвязка. Они повторно решали по теме, можно ли отвечать клиенту, хотя смысл,
маршрут и действие уже определяет основная модель. Публичный Telegram теперь
показывает любой полезный текст, который прошёл единый выходной гейт; ложное
обещание несуществующей передачи менеджеру проверяет уже существующий выходной
валидатор.

Проверка публичной передачи добавлена новым кодом в существующий смысловой
LLM-верификатор, а неполная regex-проверка удалена из публичного транспорта.
Верификатор сохраняет полезную часть через штатную регенерацию; при его
недоступности публичный текст не считается проверенным. Новый модельный вызов,
классификатор или слой не создавались.

Новых regex, marker-таблиц, флагов, зависимостей и модельных вызовов нет.
Direct-path inventory уменьшился `692 -> 690`: удалены только две одинаковые
таблицы матрицы автономности, остальные изменения снимка являются сдвигом строк.
После уборочного аудита также удалены три невызванные функции старой повторной
route-регенерации и передача `autonomy_policy` в model-owned context. Они не
участвовали в runtime; inventory остался `690`, изменились только координаты.

## Разрешённое обновление 2026-08-30: воспроизводимая историческая проверка

В сборщик контекста добавлен явный `evaluation_day`, используемый только
тестами исторических снимков. По умолчанию runtime по-прежнему берёт текущую
дату и применяет тот же freshness-гейт. Новых regex, marker-таблиц и правил
понимания клиентского текста нет; frozen snapshot обновлён только из-за сдвига
координат существующих 690 строк.
