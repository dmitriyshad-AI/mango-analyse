...
ANALYSIS_REASON_RU = {
    ...
    "commercial_demo_material_missing": (
        "обещанный демоурок или материал не получен; РОПу нужно проверить продолжение продажи"
    ),
    "commercial_course_followup_pending": (
        "после обсуждения курса ожидается продолжение контакта; РОПу нужно проверить следующий шаг"
    ),
    "commercial_payment_outstanding": (
        "по обучению осталась ожидаемая оплата; РОПу нужно проверить поступление и следующий шаг"
    ),
    "commercial_retention_risk": (
        "в разговоре об оплате или договоре есть риск отказа; РОПу нужно проверить удержание клиента"
    ),
