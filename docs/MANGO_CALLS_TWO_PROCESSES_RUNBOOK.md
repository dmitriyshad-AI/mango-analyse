...
При системной недоступности Metal используется явный `asr_mode=gigaam_fallback`: один GigaAM на CPU, затем Resolve и Analyze. Режим попадает в отчёт и не выдаётся за двойное распознавание.
