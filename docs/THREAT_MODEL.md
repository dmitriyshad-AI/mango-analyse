...
### Q4c-2 Generic Next Step After Specific Latest Call

Общий следующий шаг `Отправить материалы` / `Отправить информацию` / `Отправить письмо` не является достаточным, если последний звонок содержит конкретный артефакт действия: чек, квитанция, договор, сайт, QR-код, Telegram/email, маткапитал, налоговый вычет, оплата/счет/рассрочка, конкретная смена или заезд.

Такие строки должны либо получать более конкретный next step (`запросить чек в Telegram`, `отправить договор`, `уточнить условия оплаты маткапиталом`), либо уходить в manual-review перед live writeback. Это общий класс, не список телефонов.
