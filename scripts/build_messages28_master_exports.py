...
MASTER_CONTACTS_HEADERS = [
    ...
    "Нужна ручная проверка",
    "Коммерческая проверка",
    "Коммерческая проверка подтверждена",
...
def main() -> int:
    ...
    for source_filename, base in sorted(
        ...
        call_row = {
            ...
            "Коммерческая проверка": _safe_bool_text(bool(flat and flat.get("commercial_review"))),
    ...
    for phone, calls in sorted(by_phone_calls.items(), key=lambda item: item[0]):
        ...
        contact_row = {
            ...
            "Коммерческая проверка": _safe_bool_text(any(row["Коммерческая проверка"] == "Да" for row in calls)),
            "Коммерческая проверка подтверждена": "Нет",
