...
5. После запуска требовать: ровно один Wappi PID, `live_truth=PASS`, loaded SHA
   равен `HEAD`, heartbeat свежий, профиль `pilot_gold_v1`, legacy heuristic
   resolver выключен, authoritative exact Wappi-widget resolver включён,
   `all_personal_mode=true`, клиентская отправка выключена. Затем проверить один
   новый менеджерский черновик без отправки клиенту.
...
Решением D-116 публичный Telegram возвращён как тонкий адаптер к текущему
`SubscriptionLlmDraftProvider`: отдельный процесс и токен на Фотон и УНПК,
только личный текст, статический `/start` сообщает об ИИ. Старый публичный
runner остаётся запрещён безусловно. Содержательный модельный текст отправляется
при финальном `authoritative_output_gate=pass`, если нет ошибки и маршрут не
`blocked`. `draft_for_manager`/`manager_only` сохраняют необходимость
человеческого действия, но сами по себе не скрывают безопасное объяснение
клиенту. Публичный контур не пишет в AMO, поэтому не обещает, что уже передал
вопрос менеджеру; иначе уходит фиксированный честный ответ без служебного текста.
Структурная память диалога хранится атомарно в отдельном state-файле каждого
бренда и обновляется только после успешного `sendMessage`.
Codex запускается из изолированного каталога, чтобы не подмешивать проектные
инструкции в клиентский ответ. Постоянный `403` одного закрытого чата
фиксируется и не останавливает обслуживание остальных клиентов бренда.
`/start` и вложения в смысловую память не входят. При аварии ровно между
принятием сообщения Telegram и записью state возможен один повтор ответа.
...
Релиз содержит решения владельца от 13.08.2026 и расписание с явным
`valid_until=2027-05-31`: оно действует до этой даты. Факты о наличии мест
действуют только в день проверки и требуют штатной перепроверки источника.
Не подменять snapshot старыми v6.3/v6.7 из исторических документов. Бот может называть
