...
### D-109. Семейный граф не отменяет канонического владельца слабой исторической ссылкой

`family_graph_refresh` использует тот же результат
`authoritative_tallanto_student_owners()`, что импорт денег и посещений. Если у
student ID есть единственный точный владелец, старая `ambiguous`-ссылка другого
клиента не делает этого владельца конфликтным повторно. Ошибочный держатель
остаётся заблокированным.

Если точного владельца нет, найдено два exact/manual владельца или открыт прямой
`tallanto_identity_conflict`, все затронутые клиенты остаются конфликтными. Общий
контакт может доказывать семью, но не переносит события одного ребёнка другому.
Изменение применяется только к производному семейному графу; сырые события не
удаляются и рабочий reader автоматически не переключается.

### D-110. Старый Tallanto ambiguity переоценивается по exact ID и текущему событию

Открытый `tallanto_identity_ambiguous` перестаёт блокировать клиента только
если в нём ровно один student ID, канонический resolver возвращает одного
exact/manual владельца и существует ровно одно активное событие того же
`most_abonements` или `most_finances` с тем же владельцем и contact ID.

Единицей решения является полный конфликт с каноническим `conflict_id`, а не
общий `source_ref`. Конфликт с двумя student ID, direct
`tallanto_identity_conflict`, другой модуль или несовпадающее событие остаются
открытыми. Сохранённые conflict ID, тип и refs проверяются до импортных записей;
конфликтные обновления атомарны. Сырые события не удаляются и не переносятся.

На фиксированном staging-клоне правило закрыло 1 902 из 1 925 старых TIA;
остаток состоит из 17 direct-конфликтов и 6 конфликтов с двумя student ID.
Повтор закрыл 0. Это чинит производную блокировку, но не отменяет независимые
семейные проверки: `family_link_status` и отбор `purchase_history` принимаются
отдельно.

### D-111. Закрытый D-110 пересчитывает профиль, а исключённая улика не блокирует семью

После закрытия D-110 профиль `ambiguous` становится `strong` только если у
клиента ровно один Tallanto student ID, один exact/manual владелец, одно
совпадающее активное событие и нет другого открытого identity/family-конфликта.
Проверка открытых и закрытых конфликтов общая и выполняется пакетно. Профиль
обновляется штатным store вместе с `record_json`, hash и audit; `family_graph`
не переопределяет статус профиля самостоятельно.

Статус помечается как принадлежащий D-110. Если доказательство исчезает,
появляется второй владелец или новый блокирующий конфликт, только созданный
этим правилом `strong` возвращается в `ambiguous`. Независимый `strong` другого
источника без D-110-метки не понижается. Метка принадлежности сохраняется общим
`upsert_customer`, даже если следующий канонический writer прислал профиль без
неё; удалить метку можно только явным `null` при обратном переходе.

Строка семейного графа `excluded/suspicious_child_name` остаётся в базе как
улика, но не блокирует соседнего уверенного ребёнка. Семья только с `excluded`
или с любой `ambiguous/needs_review/unknown` строкой остаётся закрытой.

На фиксированном staging-клоне правило пересчитало ровно 235 профилей, оставило
33 владельца с другими открытыми конфликтами заблокированными и после
семейного rebuild изменило ровно 235 основных строк
`ambiguous/low -> confident/high`. Первичные события, identity links и покупки
не менялись; клиентских и внешних записей не было. Это исправление слоя данных,
а не обещание видимости всех 235 в AMO: при текущем бренде 122 семейные проекции
стали `single`, 113 требуют уточнения; exact AMO-опора есть только у семи.

### D-112. Составные классы и предметы не должны расти при повторном family rebuild

Сохранённые `grades_json` и `subjects_json` являются производным входом
следующего семейного rebuild. Поэтому оба поля перед объединением раскладываются
одним существующим splitter по `; , / |`, а затем дедуплицируются. Иначе строка
классов повторно попадала в себя и меняла производную таблицу на каждом запуске.

На фиксированном staging-клоне два последовательных rebuild дали одинаковые
хэши всех строк `family_members_v1`, `family_links_v1`,
`timeline_event_family_attributions_v1` и
`opportunity_family_attributions_v1`. Первичные таблицы не менялись.

### D-113. Модель является единственным владельцем смысла P0 в черновиковом пути

Экзамен M1 на 815 случаях дал модели `FN/FP=7/19` против `130/82` у старого
regex-классификатора при нуле ошибок провайдера. Владелец решил не повторять
модельный экзамен: целевой путь формирует черновик менеджеру и ничего не
отправляет клиенту автоматически.

В живом direct path текущая реплика больше не блокируется и не переводится в
P0 по словарю, теме или `answer_safety_classifier`. Решение принимается по
физически присутствующим `is_p0` и `p0_kind`, которые модель возвращает в том
же основном вызове. Отсутствующее или невалидное поле остаётся fail-closed и
отправляет результат менеджеру. Подтверждённая P0-защёлка предыдущего решения,
аудит и выходные проверки обещаний, фактов и принадлежности клиента сохраняются.

Старый regex допустим только как offline/legacy-измеритель и не может менять
маршрут, текст или метаданные решения живого модельного пути. Новые словари,
regex-бэкстопы, модельные вызовы и постоянные флаги для этого переключения не
добавляются.

## 2026-08-07

### D-114. Ожидаемая непривязанная посещаемость видима, но не замораживает freshness

`identity_unmatched_expected` означает, что Contact API вернул полную строку,
но для неё пока нет точной Tallanto/AMO-связи. Такое событие не получает
`customer_id`, не создаёт attendance-факт и остаётся открытым low-severity
конфликтом. Общая class-based очередь повторно проверяет его независимо от
глобального курсора, поэтому эта штатная ситуация не блокирует freshness.

`identity_infrastructure_gap` означает неполный Contact fetch и остаётся
блокером: шаг имеет `partial`, курсор не продвигается и latest не публикуется.
Список блокирующих причин принадлежит одному месту в attendance importer;
отчётный boolean вычисляется из этого списка, а не дублирует решение литералом.
...
### D-115. Основная модель владеет смыслом и действием в каноническом Wappi-черновике

В профиле `pilot_gold_v1` один существующий direct-path вызов определяет смысл
реплики, P0-поля, `requested_action` и текст черновика. Существующий model-driven
retriever выбирает факты. До обоих вызовов удаляются старые keyword/regex-планы,
предвыбранные темы и факты, поэтому они не могут переопределить решение модели.

Детерминированный слой после модели не выбирает новый intent, тему или действие.
Он сохраняется как выходная проверка: может очистить опасный текст, понизить
маршрут или закрыть результат при неподтверждённом факте, чужом клиенте, P0,
ПДн или запрещённом обещании. Подтверждённая P0-защёлка, точная identity-привязка
и read-only snapshot не удаляются.

При явном отключении любого из трёх условий (`intent_model_led`, `llm_retrieve`,
`retriever_model_driven`) старый контекст не меняется. Нового флага, зависимости
или модельного вызова нет. Три обезличенных read-only прогона реального provider
подтвердили выбор нужного факта, активный `requested_action`, конечную заметку
менеджеру и ноль внешних записей.

### D-116. Сезонный MVP возвращает публичный Telegram тонким адаптером

Решение владельца от 13.08.2026 отменяет D-105: два публичных Telegram-бота
Фотон и УНПК запускаются сегодня и прямо в статическом `/start` называются
ИИ-помощниками. Допустимы небольшие смысловые ошибки, которые исправляются по
живому трафику. Абсолютные границы: не смешивать клиентов и бренды, не выводить
внутренний служебный текст и не разрешать Wappi отправлять клиенту.

Старый 2 042-строчный Telegram-runner не возвращается. Разрешён только тонкий
адаптер текущего `main` поверх существующих `build_pilot_context_payload` и
`SubscriptionLlmDraftProvider`: один процесс и токен на бренд, только личный
текст. Модельный текст уходит клиенту исключительно при конечном
`route=bot_answer_self_for_pilot`, отсутствии ошибки и финальном
`authoritative_output_gate=pass`; во всех остальных случаях клиент получает
фиксированный честный ответ без менеджерского текста. Wappi остаётся только
контуром AMO-черновиков.

## 2026-08-14

### D-117. Публичный Telegram переиспользует каноническую память диалога

Публичный адаптер не создаёт собственную историю сообщений. Для каждой пары
`бренд + chat_id` он хранит существующий `DialogueMemory` в существующем
атомарном `DraftLoopState`: последние реплики, известные условия, открытый
вопрос и накопительную сводку. Память, отметка обработанного update и Telegram
offset записываются одной контрольной точкой только после успешного
`sendMessage`; старый offset-файл используется только для однократной миграции.

Безопасная заглушка сохраняется как реально отправленная реплика, но не закрывает
вопрос клиента как отвеченный. Вызов Codex из публичного процесса всегда
изолирован от проектных инструкций и рабочего каталога. Telegram API не даёт
ключ идемпотентности для `sendMessage`, поэтому при аварии ровно после принятия
сообщения Telegram, но до записи state, допустим один повтор. При транспортной
ошибке или HTTP 5xx сохраняется уже готовый текст; после перезапуска он
отправляется ещё не более одного раза без повторного вызова модели. Смешение
чатов или брендов недопустимо. Новый store, feature flag, зависимость и второй
Telegram runtime не создаются.

### D-118. Публичный Telegram показывает полезный проверенный текст независимо от служебного маршрута

Решение владельца от 14.08.2026: служебные `draft_for_manager` и
`manager_only` больше не являются самостоятельным запретом клиентского текста.
Публичный транспорт отправляет любой содержательный модельный текст, если нет
ошибки, маршрут не `blocked`, общий `authoritative_output_gate` выполнился и
существующий смысловой verifier вернул `pass` либо `pass_after_regen`.
Промежуточный прогон 16 диалогов показал рост видимого текста `5 -> 13` после
снятия route-блокировки, но не является финальным semantic-pass: он снят до
добавления проверки ложной передачи менеджеру. Финальный прогон должен быть
привязан к чистому SHA и хранить оба выходных вердикта.

Маршрут модели сохраняет смысл: он показывает, требуется ли действие человека,
и управляет памятью/P0. Это не разрешение обещать действие. У публичного
Telegram-контура нет подтверждённой передачи в AMO, поэтому его direct-path
prompt прямо запрещает утверждать, что вопрос уже передан, менеджер уже
уведомлён или выполнит действие. При нехватке факта бот сначала даёт безопасную
пользу и предлагает подтверждённый способ связи с учебным центром. Wappi
по-прежнему только создаёт AMO-черновик и ничего не отправляет клиенту.

### D-119. Память Wappi используется только после доказанной непрерывности личности

Ключ `profile_id + chat_id` достаточен для последовательности сообщений, но не
доказывает, что Wappi никогда не переиспользовал чат для другого AMO-контакта.
Если сохранённой пары нет и authoritative resolver заново определяет точный
контакт, старая память этого chat_id не попадает в промпт. После подтверждённой
AMO-заметки она заменяется памятью текущего наблюдавшегося диалога. При наличии
старой пары несовпадение contact_id остаётся конфликтом и блокирует запись.

## 2026-08-24

### D-120. Каждый Wappi-диалог имеет AMO-контакт и минимум одну сделку

Решение владельца и проверка интерфейса Wappi: у каждого диалога в правой
панели есть ссылка на контакт amoCRM и минимум на одну сделку. Поэтому
`REGISTRY_MISSING`, `LINK_EXTRACTION_GAP`, пустые `amo_contact_ids` или
`amo_lead_ids` означают дефект извлечения или неполный снимок, а не бизнес-факт
«контакта/сделки нет».

Для кампании такой диалог не теряется и не объявляется неподходящим. Он получает
статус `ПРОВЕРКА`, пока штатный extractor не восстановит точные ID из нативной
связи Wappi. В колонке «Открыть» допустима только доказанная ссылка на AMO-контакт
или сделку; угадывать ID по телефону, имени или частичному совпадению запрещено.

Скриншоты интерфейса с видимыми ссылками на контакт и сделку сохранены в
локальном audit pack текущей Wappi-кампании. Это решение определяет ожидаемый
инвариант продукта; фактическую полноту конкретной выгрузки всё равно проверяет
машина.

## 2026-08-27

### D-121. Подтверждённая претензия исключает строку из рассылки

Решение владельца от 27.08.2026 заменяет прежний маршрут претензий в
`МЕНЕДЖЕР СЕГОДНЯ` именно для рассылочной кампании. Если первичные факты
подтверждают претензию или жалобу клиента, строка получает `НЕ ПИСАТЬ`, не
попадает в очередь рассылки, а сообщения 1-5, даты и подпись остаются пустыми.
AMO-ссылка, фактическое описание претензии и ручные колонки X:Y сохраняются.

Гипотетическая формулировка (`если появится`, `риск жалобы`, `претензии нет`)
не считается подтверждённой претензией. Перед каждой записью исполнитель заново
читает строку Google и не возвращает подтверждённую претензию в
`МЕНЕДЖЕР СЕГОДНЯ`. Wappi, AMO и Tallanto в этом процессе используются только
для чтения; внутренние сервисные и юридические действия идут отдельным путём.

## 2026-08-30

### D-143. Постоянное разрешение на обезличенный аудит Claude Opus 5 и Fable 5

Решение владельца: Claude Opus 5 и Fable 5 разрешено без повторного
согласования передавать обезличенные манифесты и агрегаты, внутренние пути,
код и документацию пакетов проекта. Разрешение постоянно действует для
архитектурного, технического и бизнес-аудита.

Запрещено передавать ПДн, секреты, токены, строки клиентских данных, сырые
диалоги, клиентские таблицы и базы данных. Расширение этих границ требует
нового явного решения владельца; внутри зафиксированных границ повторно
спрашивать подтверждение не нужно.

### D-144. Customer Timeline получает одного писателя через восстановление seed M1

Точный кандидат M1 `1b3709f693239cedb66219793623f62202c11498` принимается
как кодовая основа единой ночной линии. Данные нельзя объединять слиянием двух
SQLite-файлов: M4 восстанавливает полный проверенный seed M1, затем повторно
проигрывает только доказанный хвост источников M4 штатными идемпотентными
адаптерами. После cutover остаётся одна служба и один writer.

Запись M4 запрещена до отдельного доказательства остановки writer M1: receipt
должен назвать процесс, расписание, время остановки и SHA переданного seed.
Пакет M1 от 30.08.2026 прямо помечает M4 как `PENDING_ACTIVATION`, поэтому
текущая задача интегрирует код, но не переключает runtime и не отключает старую
службу M4.

Канонический receipt всегда остаётся неизменяемым `TRANSFERRED`: вручную
объявить `M4_WRITER_ACTIVE` нельзя. Первый полностью зелёный цикл сам создаёт
отдельный `M4_WRITER_ACTIVATION.json`, привязанный SHA к исходному receipt и
первому `service_report.json`; следующие циклы принимают только эту пару.
Writer не запускается из worktree с изменёнными tracked-файлами.

Перед runtime-cutover обязательны три машинных гейта: compact reader укладывается
в заданный конфигом бюджет 8 GiB; фактическая видимость проверяется каноническим
reader-policy, а не сырым `allowed_for_bot`; flip принимает только SQLite рядом
с совпадающим manifest/SHA и автоматически возвращает проверенный backup при
ошибке после подмены. Ослабление этих гейтов требует отдельного решения владельца.
Manifest также обязан нести полный доказанный lineage источников: непустые
source counts и cursors, закрытый required-sources check и фактический SHA кода
сборщика. Wappi draft-loop обязан дочитывать короткую страницу при `has_more=true`;
диагностический `chat_limit` несовместим с live-write.

## 2026-09-02

### D-145. Единый writer M4 заменяет старое расписание после двух зелёных циклов

Решение владельца: старое расписание Customer Timeline на M4 разрешено
персистентно отключить. Новое единое расписание можно установить только после
двух последовательных зелёных ручных циклов на свежем восстановлении frozen
seed M1, проверки compact-reader и независимого аудита без P0/P1. До этого
никакое расписание writer не запускает. Это решение заменяет требование трёх
циклов из D-091 только для объединённого M4 writer; требования audit pack и
отдельной проверки результата сохраняются.

Почтовые дневные задачи после этой смены только скачивают и готовят вход.
Каноническую staging-БД меняет исключительно единый nightly. Первый зелёный
цикл сам создаёт activation-квитанцию; ручное объявление writer активным
запрещено.

### D-146. Wappi сохраняет только доказанный прогресс и не требует ручной перепечатки

Входные manual/auto pair-карты каждого nightly-run фиксируются неизменяемыми
копиями, адресованными по SHA. Совместимость перенесённого M1 checkpoint
обязательна только до первой активации M4. После активации повреждение или
смена checkpoint запускает штатное полное чтение конкретного профиля и не
останавливает остальные источники до сети.

При дрейфе offset-пагинации один проблемный чат перечитывается целиком один
раз. Если дрейф сохраняется, в БД и checkpoint разрешено записать только уже
полностью прочитанные и проверенные чаты; проблемный чат и дата полного аудита
не подтверждаются, весь цикл остаётся неготовым к публикации. Если до сбоя не
подтверждён ни один чат, запись и checkpoint не меняются. Ручная перепечатка
checkpoint не является штатным способом восстановления.

### D-147. Владение writer не равно зелёной приёмке цикла

`M4_WRITER_ACTIVE` доказывает только то, что M1 остановлен и данную staging-БД
теперь меняет единственный writer M4. Если первый безопасный цикл уже записал
доказанный частичный прогресс, статус владения фиксируется и при
`pass_with_notes`, чтобы база не осталась в неопределённом состоянии между
seed и следующим запуском.

При этом `pass_with_notes` не считается зелёным циклом, не обновляет
last-success штатной обёртки и не приближает установку расписания. Два цикла из
D-145 обязаны иметь `data_quality_status=pass`. Каноническую staging-БД с
ownership-квитанцией общий `Store` открывает на запись только внутри scope
единого nightly; прямые операционные CLI остаются read-only/STOP для неё.

### D-148. M4 принимает владение непосредственно перед первой записью

Решение владельца о переносе writer на M4 уже подтверждено остановочной
квитанцией M1. Поэтому активация M4 фиксируется не после результата первого
цикла, а на последней безопасной границе перед первой записью: единый lock уже
взят; frozen seed, stop-квитанция, код и точный файл конфига проверены; WAL пуст;
перенесённый Wappi-checkpoint совместим в read-only режиме. Только после записи
`M4_WRITER_ACTIVATION.json` открывается scope единого writer и начинаются шаги.

Почему: результат цикла оценивает качество данных, а не право владения. Авария
или доказанный частичный прогресс после первой записи не должны превращать
валидный перенос writer в невосстановимый тупик. Это уточняет формулировки
D-145 и D-147: вручную объявлять M4 активным по-прежнему нельзя, но канонический
nightly создаёт активацию до первой записи, а не ждёт зелёного отчёта.

Дочерний процесс получает право записи только для точного пути этой staging-БД
и только пока его реальный родитель одновременно удерживает общий nightly-lock
и связанный owner-proof lock; PID владельца проверяется через ОС. Один лишь env,
незанятый lock или lock чужого процесса права не дают. Строгий SHA файла
конфига сохраняется; квитанция выписывается непосредственно перед первым
циклом. `data_quality_status=pass` остаётся обязательным для каждого из двух
зелёных циклов и для установки расписания.

### D-149. `latest` публикуется только из цикла с чистым качеством

Версионный отчёт частичного или деградированного запуска сохраняется для
диагностики, но указатель `latest_customer_timeline_snapshot.json` обновляется
только при `data_quality_status=pass`. Статусы `partial` и `pass_with_notes` не
могут незаметно стать текущим снимком ни для оператора, ни для следующего шага
публикации. Это приводит фактическую публикацию в соответствие с D-145–D-147.

Два зелёных цикла подтверждают исправность объединённого writer и доступность
его настроенных локальных источников. Они отдельно не доказывают свежесть
upstream-контура обработки звонков; до такого утверждения нужна собственная
квитанция свежести Calls. Это ограничение указывается в каждом отчёте r5.

### D-150. Почтовый архив переиспользует доказанные UID внутри эпохи ящика

Штатный read-only IMAP-инкремент не скачивает повторно тело уже сохранённого
письма, только если совпадают аккаунт, ящик, UID и `UIDVALIDITY`. Смена
`UIDVALIDITY` запрещает такое переиспользование: одинаковый UID новой эпохи
считается другим источником. Полные письма читаются пакетами через
`BODY.PEEK[]`; при несовместимости сервера код возвращается к одиночному
`BODY.PEEK[]`, не меняя почтовый ящик.

Причина: историческая догрузка иначе повторно передавала тысячи уже
архивированных вложений и занимала часы. Решение ускоряет только чтение и не
ослабляет полноту, дедупликацию по содержимому или read-only контракт IMAP.

Уточнение 05.09.2026 после независимой проверки четырёх поздних коммитов:
`account_label` не доказывает аккаунт. Пропуск привязан к SHA кортежа
сервер/порт/логин в `message_sources.account_key`; пароль туда не входит.
Регистр логина сохраняется, так как сервер вправе различать учётные записи.
Старые строки без ключа не повышаются предположительно: только повторный FETCH
в текущем временном окне создаёт доказанную запись источника. Это может добавить
строку `message_sources`, но не дубликат письма или события по SHA.

Пакетный `IMAP4.error`/NO переводит текущее подключение на одиночное чтение;
частичный OK сохраняет полученных соседей и дочитывает отсутствующий UID.
`IMAP4.abort` остаётся ошибкой соединения для штатного восстановления с проверкой
эпохи. Старый индекс достаточен; новый загрузчик, индекс и флаг не нужны.

Эксплуатационное решение для r5: после двух pass и проверки compact установить
одну последовательную задачу `mail-chain && nightly-warehouse`. Отдельные
почтовые расписания не включать: временной зазор между двумя заданиями не
гарантирует готовности почты. Используются существующие команды и их блокировки.
Состояние приёмки и результаты циклов хранятся в локальном плане r5; это решение
само по себе не является подтверждением установки расписания.

## 2026-09-06

### D-151. R5 ремонтирует поисковые ключи один раз на пакет изменений

При первом отсутствующем ключе существующего события или чанка внутри
`bulk_write` Store восстанавливает соответствующую служебную карту из реальных
FTS-rowid одним SQL-проходом. Далее пакет использует поиск по первичному ключу,
а не повторно просматривает весь поисковый индекс для каждого объекта.
Полнота карты действует только в текущей внешней транзакции; commit/rollback
сбрасывают отметку. Обычная связанная дельта и неизменный повтор ремонта не требуют.

Почему это минимум: пропуск orphan-проверки может оставить старый текст в
поиске; полный rebuild на каждый missing повторно обрабатывает весь текст.
Уже существующих карт и deferred rebuild достаточно, новых таблиц/флагов нет.
Дубли FTS-ID и legacy NULL-rowid сохраняют защитный путь полной пересборки;
вне bulk прежний fallback не меняется. Переписывается только производная карта,
основные события, связи клиентов и фильтры видимости не затрагиваются.

Синтетический замер реального Store: 10000 индексированных событий и 1000
изменённых unlinked-событий, 1000 сканов -> 1 ремонт карты; 1,7625 -> 0,1985 с.
Повтор: 0 обновлений, 1000 duplicate, 0 сканов/ремонтов, 0,0816 с. Это не замер
всей службы и не доказательство свежести Calls. Первое изменение с missing
в следующей транзакции вправе снова потребовать один проход служебной карты.

Новый код не применять к активному r5 через перепечатку ownership/activation:
исторические квитанции остаются неизменными. Переход версии M4 writer, Wappi-
неоднозначности и два строгих зелёных цикла остаются отдельными пунктами цели.

### D-152. R5 сохраняет доказательства личности и историю обновления writer

Wappi сравнивает только явные active alias текущего tenant. Канонические
записи A->A, присутствующие в M1, означают отсутствие изменения; split,
развилки, настоящие циклы и отсутствующие цели не дают уверенной привязки.
Кандидаты и все бренд-свидетельства сохраняются. Неоднозначный контакт нельзя
незаметно заменить одним владельцем сделки; это относится и к позднему AMO.
Alias-сравнение само не переносит физического владельца события/сделки.
Отсутствие нового предложения привязки не равно новому конфликту; существующий
карантин сохраняется. Настоящее противоречие двух resolved остаётся запретом.

Проверка кэша (без API/записи):3772 строки,2687 ->1551 resolved,2221 pending;
1136 понижений обусловлены явной неоднозначностью contact/lead. Это не рост
покрытия и не доказательство1136 ошибочных старых привязок. Число41 ложных/
неполно подтверждённых owner-conflict больше не подменяет проверку личности.
Исторические конфликты и записи этой правкой не удаляются.

Обновление активного M4 выполняется отдельной ручной командой существующего
CLI под wrapper/service/writer locks. Единственный новый артефакт на переход
версии — неизменяемая code-release квитанция, связанная с исходными ownership,
activation, конфигом и каноническим review всего диапазона OLD->NEW.
Первоначальные квитанции не переписываются; новый run не возобновляет старый
под чужой версией, но использует checkpoints источников. Родительские переходы
и symlink в путях квитанций запрещены. Обычная команда не имеет bypass-флага.
Более простые варианты (перепечатать ownership или убрать HEAD-check) отвергнуты:
они уничтожают доказательство единственного writer. Отдельного current.json,
таблицы, зависимости или службы не добавлено.

Гарантия относится к новому входу запуска. Исторический checkout не знает
этого validator; его прямой запуск запрещён эксплуатационным правилом,
старое расписание остаётся выключенным. Code-release не означает успешный
цикл, готовность compact, включение расписания или публикацию production.
Два строгих pass и реальный reader остаются обязательными.

### D-153. R5 завершает доказанные порции AMO и Wappi без полного повторения

Причины последнего partial подтверждены ограниченными read-only пробами:
у AMO leads изменился состав страницы (16 из20 ID не было в кэше); у Wappi
один идентичный messageID повторялся между страницами100/100/3. Каталог Wappi
был доказан. Удаление полей из AMO-anchor и ослабление привязки отвергнуты.

AMO использует прежний importer/checkpoint. Доказанные завершённые endpoint
переиспользуются без GET и сохраняются независимо. Дрейфующий endpoint
переходит на непрерывные временные порции внутри прежней верхней границы.
Переполненная порция уменьшается, уменьшение переживает рестарт; дешёвая
успешная порция ограниченно увеличивается, чтобы не читать пустые сутки
посекундно. Все новые страницы и терминальность перепроверяются; legacy-
префикс проверяется до и после продолжения. Недоказанные строки остаются
в локальном checkpoint, не становятся подтверждённым результатом.

На endpoint: не болееN страниц данных иN проверок, плюс2 граничных пробы.
Локальные повторные вызовы при429 входят в лимит2N+2; скрытые retries MCP
не объявляются измеренными HTTP-запросами. Счётчики названы client_get_calls
и retry_get_calls. Исчерпанный бюджет/одна недоказанная секунда не даютPASS.
Курсоры и применение в БД по-прежнему требуют полноты всех четырёх endpoint.

Wappi схлопывает межстраничный повтор только при равенстве смысловой подписи
того жеID. Подпись всей сырой страницы и проверка всех страниц сохраняются;
offset считает сырые строки. Разное содержимое и повтор без продвижения
останавливают чат. Плохой full-чат не лишает следующие чаты попытки проверки.
Частичная фиксация допустима только для независимо доказанного материала
при изолированном full-history drift и подтверждённом каталоге. Ошибка
хвоста, происхождения или исторической сверки остаётся блокирующей.
Ни карантин, ни владелец старого события, ни бот-видимость не повышаются.

Дополнительно согласованы два существующих чтения карантина: локальный
отбор раньше смотрел только identity_authority, а финальный загрузчик также
pending_attribution+причину конфликта. Теперь оба используют одно прежнее
финальное правило. Это исключает слабые повторные предложения из отбора;
счётчик/гейт настоящего конфликта владельцев не изменён. Проверены все три
формы хранения карантина и неизменность исходной строки. Возможность новых
отказов при сетевом чтении не объявляется устранённой без реального прогона.

Архитектор выявил пустую нетерминальную страницу AMO и недоказанный Wappi-
хвост до вызова helper; оба класса закрываются отдельными регрессиями.
Более простой новый импортёр/новое расписание отвергнуты как дублирование.
Изменяется только чтение и доказательство полноты в существующих владельцах;
новых таблиц, флагов и зависимостей нет. Новая версия требует отдельного
code-release отac2983db; активацияM1/M4 и прежние квитанции не переписываются.
Кодовая приёмка не означает два зелёных цикла, готовность compact или запуск
расписания. Эти проверки выполняются отдельно на текущем r5, без новогоseed.

Уточнение после release175b0aad: консервативная разметка выявила156строк
обычного кода вместо лимита150. Защитные162строки не переклассифицировались.
Выбран минимальный вариант: одна tail/full развилка вместо шести повторных
тернарных присваиваний и повторной установки defaults в диагностике первого
сбоя.20→13строк, обычный код149. Все поля и счётчики сохраняются. Альтернатива
отдельного разрешения превышения отвергнута как ненужная. Правка выполнена
после завершения source-only AMO, не на работающем writer; новыйHEAD требует
обычной release-квитанции. Прогресс API и исходные данные не переписываются.

### D-154. R5 ищет вторичные зависимости сигналов один раз на пакет

06.09.2026. Продолжение переноса единственного писателя по решению Дмитрия.
Причина: каждый отзыв связи события с клиентом повторно перебирал JSON всех
сигналов tenant; существующий частичный индекс не покрывает этот запрос.
Сохраняется общий владелец Store, новый импортёр и постоянная таблица не нужны.

Внутри bulk_write лениво строится временная индексированная карта
tenant/event/signal. Карта только сужает кандидатов; окончательное решение
принимают прежние проверки живого JSON, владельца и прямой ссылки. Новые и
изменённые сигналы пополняют готовую карту через существующий _upsert_record;
дубликаты её не трогают. Отозванные ссылки могут остаться кандидатами, но не
становятся действующими связями. Вне bulk прежний путь сохраняется.

При неудаче пополнения готовность tenant снимается; перехваченная внутри
пакета ошибка не позволяет использовать неполную карту. На внешних границах
пакета готовность сбрасывается. Построение TEMP исключено из интервала
total_changes при отзыве источника: временные строки не являются приростом
клиентских данных. Последнее выявлено независимым архитектором и обязательно
проверяется отдельным тестом вместе с rollback/nested/repair/tenant кейсами.

Отвергнуты: multi-source индекс как единственный фильтр (теряет singleton),
постоянная таблица с миграцией (лишний контракт), invalidate/rebuild после
каждого signal (повторяет полный проход в смешанном потоке). Ошибки JSON не
скрываются. Построение без owner-фильтра может раньше обнаружить некорректный
legacy JSON; это fail-loud, не разрешение игнорировать данные ради скорости.
Прежний частичный индекс здесь не удаляется; его необходимость требует
отдельной проверки потребителей и согласованной уборки.

Флаги бота, карантин, strong-порог, курсоры, сетевые входы и постоянная схема
не меняются. Замер захваченного runtime SELECT на read-only r5 даёт одинаковые
строки старого и нового запроса; :memory: карта и прогретый кэш не доказывают
сквозную скорость настоящего Wappi-прогона. Она проверяется после code-release,
не выдаётся заранее за закрытие цели. AMO/seed заново не загружать.

Бюджет реализации: +42/-3 нетестовых строк Store, один внутренний helper,
одна временная таблица, новых runtime-файлов/флагов/зависимостей нет. Два
строгих цикла, compact, приёмка обоих Mac и одно расписание остаются отдельными
критериями. Паспорта и уборка копий сохранены в общем плане, не смешиваются
с этой кодовой правкой. Номер D-154 проверен по decision logs 25 локальных веток.

### D-155. AMO восстанавливает дрейф страниц в том же вызове

06.09.2026. Продолжение активной цели Дмитрия, без новых выгрузок всей базы.
В двух полных циклах e177f0ea единственным обязательным отказом был AMO leads.
Отдельный recovery завершался, но следующая дельта снова шла старым путём.
Выбран вызов существующего оконного helper после доказанного drift, если
остался бюджет страниц. Общий счётчик запросов, верхняя граница и гейты прежние;
стабильные страницы продолжаются с checkpoint, готовые endpoint не скачиваются.

Отвергнуты: повтор всего ночника ради AMO; новый импортёр; безусловные окна
для всех новых leads. Последний вариант первоначально поддержали аудиторы,
но main нашёл тупик при стабильном плотном потоке за одну секунду. Независимый
архитектор воспроизвёл и отозвал рекомендацию. Старый тест большого backlog
не ослаблен, добавлен тест действительно одинаковых времён событий.

Неподтверждённые полные версии не становятся фактами клиента. Версии первого
чтения, несовпавшей перепроверки и граничной пробы сохраняются отдельно с
дедупом всей строки, не ID. Последний проблемный цикл каждого endpoint остаётся
в служебном ключе прежнего private checkpoint; стабильные циклы его сохраняют,
следующий инцидент заменяет. Это не новая очередь применения или бесконечный
архив. Перенос учитывает старый checkpoint без этого ключа. Аудитор нашёл три
краевых случая потери сырья; каждый закреплён регрессионным тестом.

Изменение не касается записей AMO/CRM/Tallanto, новых флагов, схемы, расписаний
или production. Квитанция первоначальной активации не переписывается: перед
следующим r5 нужен штатный code-release. Последний run141027 завершён partial
за2228.665s, WAL[0,0,0]/0, latest не изменён. Кодовый PASS не засчитывается как
один из двух строгих циклов; свежий Calls-пакет M1 остаётся отдельным входом.

Бюджет: +65/-7 строк нетестового кода, включая защиту сохранности сырья и
наследуемого checkpoint. Сам механизм inline укладывается в50 строк; защитные
изменения не режутся ради лимита. Новых файлов runtime, флагов, зависимостей
и helper-функций нет. D-155 проверен в decision logs 26 локальных веток и
согласован с параллельным исполнителем паспортов, который использует D-156.

## 2026-09-06: внешний результат ночного отчёта

Problem-ID: problem.customer_timeline.wrapper_nested_json_status.
Первый полный r5 run20260906T153346Z: service overall_status=ok,
data_quality_status=pass,14/14ok. Wrapper ложно остановился из-за предупреждения
перед JSON: общий parse_last_json возвращал последнюю вложенную диагностику.
Исправлен существующий helper последовательным JSONDecoder, без изменения
writer, конфигурации и проверок качества. Повреждённый вывод возвращает непустую
ошибку: пустой словарь позволял вызывающему взять старый успешный артефакт.

Сомнительный суффикс после JSON означает STOP. Предложение Claude терпеть его
отклонено: молчаливое восстановление противоречит однозначной границе отчёта.
Для nightly причина отказа остаётся data_quality_status_missing, для остальных
задач error; подробный stdout сохранён. Это осознанное сохранение общего гейта.
Общий mail-chain использует тот же helper; error не проходит его _stage_status.

Первый сервисный PASS засчитывается как1/2 при независимой приёмке: фактический
новый helper точно воспроизвёл --summary-only проекцию сохранённого полного
отчёта; полный конфигурационный fingerprint совпал. Старую wrapper-квитанцию
не переписывать, новую историческую проверку сохранить отдельно. Перед вторым
циклом обязательны commit/review/code-release штатным механизмом. Владение M4
не меняется, новый seed не нужен. Полный экзамен LLM не запускать.

Минимум: нетестовый diff +13/-18, тесты +50; новых файлов кода, флагов,
зависимостей0. Отвергнуты подавление warning и отдельный ночной parser, потому
что они оставляют общий дефект или дублируют механизм. Тесты:273профильных и
31почтовый, всеPASS. Это проверка класса ошибки измерителя, не обещание
актуальности всех источников и не принятие второго полного цикла.
